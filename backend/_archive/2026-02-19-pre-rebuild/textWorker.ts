...
export class TextWorker extends BaseWorker {
  ...
  protected async processTask(task: JobTask): Promise<TaskResult> {
	      ...
	      if (system === 'western' && docType !== 'overlay' && getProviderForSystem(system) === 'claude') {
	        ...
	        if (subject?.name) {
	          ...
	          for (let attempt = 1; attempt <= MAX_DIGEST_ATTEMPTS; attempt += 1) {
	            ...
	            const wound = extractWoundFromDigest(digest);
	            const woundBlock = wound
	              ? [
	                  '══════════════════════════════════════════════════════════',
	                  'THE WOUND — SPINE OF THIS ENTIRE READING (READ THIS FIRST):',
	                  wound,
	                  'Every section, every image, every insight in this reading must connect back to this one sentence.',
	            ...
	            console.log(`✅ [TextWorker] Western digest accepted (${validation.evidenceLines.length} evidence lines). Wound extracted: ${wound ? 'yes' : 'no'}.`);
	            digestAccepted = true;
	            break;
	          }
	          if (!digestAccepted) {
    ...
    if (wordCount < HARD_FLOOR_WORDS && system !== 'western') {
