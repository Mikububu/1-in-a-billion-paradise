 ...
 * Output: one paragraph, 120-180 words, naming the central narrative trigger.
...
export function buildWesternTriggerPrompt(params: {
  ...
  return [
    ...
    '',
    TRIGGER_WORD_RULE,
    'Third person. Use selective astrology language when it sharpens precision, then explain it in plain words immediately.',
    'No repair instructions. Avoid generic reassurance.',
    'Specific enough that no other chart produces this exact sentence.',
    'Make it specific and emotionally honest, not theatrical.',
    '',
    'Do not write as a mechanical placement list.',
...
export function buildWesternWritingPrompt(params: {
  ...
  return [
    ...
    narrativeTrigger,
    `Most paragraphs should connect to this ${trigger} or deepen it.`,
    `Do not force every sentence back to the ${trigger}; allow natural progression when evidence supports it.`,
    '══════════════════════════════════════════════════════════',
    '',
    'NARRATOR:',
    ...
    'SHADOW & DEPTH:',
    `- ${WRITING_shadow_RULE}`,
    '- Name the addiction potential, the manipulation patterns, the self-sabotage loops.',
    '- Do not moralize. Do not offer repair instructions. Just name what is true.',
    '',
    'STRUCTURE:',
    '- One continuous layered essay in paragraphs.',
    '- The narrative must build. Something must change between the first and the last.',
