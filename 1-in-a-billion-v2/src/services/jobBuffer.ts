...
// - no hard cap on count; retention window controls storage growth
...
const persistWithRetention = async (candidate: JobReceipt[]): Promise<JobReceipt[]> => {
    const normalized = normalizeReceipts(candidate);
    const retained = applyRetentionWindow(normalized);
...
export async function addJobToBuffer(receipt: JobReceipt): Promise<void> {
    ...
    await persistWithRetention(next);
}

export async function enforceJobBufferCap(): Promise<void> {
    const existing = await getJobReceipts();
    await persistWithRetention(existing);
}
