...
export const RootNavigator = () => {
    ...
    // Refresh entitlement when app returns to foreground so chat/matching gates stay current.
    useEffect(() => {
        if (!isAuthReady || !hasSession || env.ALLOW_PAYMENT_BYPASS) return;
        ...
        const refreshEntitlement = async () => {
            ...
            // Keep last known active/inactive state on transient failures.
            setEntitlementStatus((prev) => (prev === 'active' || prev === 'inactive' ? prev : 'error'));
        };
        ...
        const subscription = AppState.addEventListener('change', (nextState) => {
            ...
            refreshEntitlement().catch((error) => {
                if (cancelled) return;
                console.warn('⚠️ Entitlement foreground refresh failed', error);
            });
        });
