-- ===============================================================================
-- MIGRATION 033: Reduce audio task heartbeat timeout (15 mins → 5 mins)
-- ===============================================================================
--
-- PROBLEM: Previous migration (030) set audio timeout to 900s (15 minutes).
--          While better than 1 hour, it still leaves jobs stuck for 15 minutes 
--          if Replicate or the worker crashes silently.
--
-- FIX: Change audio timeout to 300s (5 minutes). Replicate chunks take ~60s
--      at most, so 5 minutes is plenty of buffer while ensuring fast recovery.
--
-- ===============================================================================

BEGIN;

-- Update the trigger function to use 300 instead of 900 for audio
...
BEGIN
  ...
  LOOP
    ...
    -- FIX: Changed from 900 (15 minutes) to 300 (5 minutes) for faster crash recovery
    IF v_audio_already_enqueued = 0 THEN
      INSERT INTO job_tasks (
        ...
        0,
        3,
        300  -- 5 minutes (was 900 = 15 minutes, still too long if worker crashes)
      );
...
-- Also update any existing pending/processing audio tasks that are stuck with the old 900 or 3600 timeout
UPDATE job_tasks 
SET heartbeat_timeout_seconds = 300
WHERE task_type = 'audio_generation' 
  AND heartbeat_timeout_seconds > 300;
