...
exports.buildChartAwareProvocations = buildChartAwareProvocations;
// ═══════════════════════════════════════════════════════════════════════════
// MAIN ENTRY POINT
// ═══════════════════════════════════════════════════════════════════════════
function buildChartAwareProvocations(personName, system, chartData, spiceLevel) {
    try {
        switch (system) {
            case 'western':
...
function parseWesternHighlights(chartData) {
    const h = {};
...
function parseHDHighlights(chartData) {
    const h = { openCenters: [], definedCenters: [], channels: [] };
    const typeMatch = chartData.match(/Type:\s*(.+?)$/m);
    if (typeMatch)
        h.type = typeMatch[1].trim();
    const stratMatch = chartData.match(/Strategy:\s*(.+?)$/m);
    if (stratMatch)
        h.strategy = stratMatch[1].trim();
    const authMatch = chartData.match(/Authority:\s*(.+?)$/m);
    if (authMatch)
        h.authority = authMatch[1].trim();
    const profileMatch = chartData.match(/Profile:\s*(.+?)$/m);
    if (profileMatch)
        h.profile = profileMatch[1].trim();
    const crossMatch = chartData.match(/Incarnation Cross:\s*(.+?)$/m);
    if (crossMatch)
        h.incarnationCross = crossMatch[1].trim();
    const openMatch = chartData.match(/OPEN CENTERS.*?:\s*(.+?)$/m);
    if (openMatch)
        h.openCenters = openMatch[1].split(',').map(s => s.trim()).filter(Boolean);
    const defMatch = chartData.match(/DEFINED CENTERS.*?:\s*(.+?)$/m);
    if (defMatch)
        h.definedCenters = defMatch[1].split(',').map(s => s.trim()).filter(Boolean);
...
function buildHDProvocations(name, chartData, spice) {
...
function parseGeneKeysHighlights(chartData) {
    const h = {};
    ...
    while ((m = sphereRe.exec(chartData)) !== null) {
        ...
        if (l.includes("life's work")) {
            h.lifesWorkKey = key;
            h.lifesWorkShadow = shadow;
            h.lifesWorkGift = gift;
            h.lifesWorkSiddhi = siddhi;
        }
        else if (l.includes('evolution')) {
            h.evolutionKey = key;
            h.evolutionShadow = shadow;
            h.evolutionGift = gift;
        }
        else if (l.includes('radiance')) {
            h.radianceKey = key;
            h.radianceShadow = shadow;
        }
        else if (l.includes('purpose')) {
            h.purposeKey = key;
            h.purposeShadow = shadow;
            h.purposeGift = gift;
        }
        else if (l.includes('attraction')) {
            h.attractionKey = key;
            h.attractionShadow = shadow;
            h.attractionGift = gift;
        }
...
function buildGeneKeysProvocations(name, chartData, spice) {
...
function parseKabbalahHighlights(chartData) {
    const h = { dominantSefirot: [], voidSefirot: [], activeKlipoth: [], voidWorlds: [] };
    const tikkunNameMatch = chartData.match(/Tikkun Name:\s*(.+?)$/m);
    if (tikkunNameMatch)
        h.tikkunName = tikkunNameMatch[1].trim();
    const correctionMatch = chartData.match(/Correction:\s*(.+?)$/m);
    if (correctionMatch)
        h.tikkunCorrection = correctionMatch[1].trim();
    const trapMatch = chartData.match(/Trap:\s*(.+?)$/m);
    if (trapMatch)
        h.tikkunTrap = trapMatch[1].trim();
    const giftMatch = chartData.match(/Gift:\s*(.+?)$/m);
    if (giftMatch)
        h.tikkunGift = giftMatch[1].trim();
    ...
    if (shadowMatch)
        h.primaryShadowAxis = shadowMatch[1].trim();
    ...
    if (domWorldMatch)
        h.dominantWorld = domWorldMatch[1].trim();
...
function buildKabbalahProvocations(name, chartData, spice) {
    const h = parseKabbalahHighlights(chartData);
    if (!h.tikkunName && h.dominantSefirot.length === 0)
        return buildFallbackProvocations(name, spice);
...
// ═══════════════════════════════════════════════════════════════════════════
function buildFallbackProvocations(personName, spiceLevel) {
    const base = `
...
//# sourceMappingURL=chartProvocations.js.map