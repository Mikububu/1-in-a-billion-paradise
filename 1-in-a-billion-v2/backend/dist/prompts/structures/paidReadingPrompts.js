...
exports.TOTAL_DOCS = exports.VERDICT_DOC = exports.NUCLEAR_DOCS = exports.SYSTEM_DISPLAY_NAMES = exports.SYSTEMS = void 0;
exports.getDocInfo = getDocInfo;
exports.buildPersonPrompt = buildPersonPrompt;
exports.buildOverlayPrompt = buildOverlayPrompt;
exports.buildVerdictPrompt = buildVerdictPrompt;
...
exports.SYSTEMS = ['western', 'vedic', 'human_design', 'gene_keys', 'kabbalah'];
...
exports.NUCLEAR_DOCS = (() => {
    const docs = [];
    let id = 1;
    for (const system of exports.SYSTEMS) {
        docs.push({
            id: `${id++}`,
            system,
            docType: 'person1',
            title: `${exports.SYSTEM_DISPLAY_NAMES[system]} - Person 1`,
            wordTarget: wordCounts_1.STANDARD_READING.target,
        });
        docs.push({
            id: `${id++}`,
            system,
            docType: 'person2',
            title: `${exports.SYSTEM_DISPLAY_NAMES[system]} - Person 2`,
            wordTarget: wordCounts_1.STANDARD_READING.target,
        });
    }
    for (const system of exports.SYSTEMS) {
        docs.push({
            ...
            system,
            docType: 'overlay',
            title: `${exports.SYSTEM_DISPLAY_NAMES[system]} - Synastry`,
            wordTarget: wordCounts_1.STANDARD_READING.target,
        });
    }
    return docs;
...
exports.TOTAL_DOCS = exports.NUCLEAR_DOCS.length + 1;
function getDocInfo(docNum) {
    if (docNum === 16) {
        return exports.VERDICT_DOC;
    }
    const doc = exports.NUCLEAR_DOCS[docNum - 1];
    if (!doc) {
...
function buildOverlayProvocations(person1Name, person2Name, spiceLevel) {
    const base = `
...
function buildVerdictProvocations(person1Name, person2Name, spiceLevel) {
    const base = `
THE FINAL QUESTIONS:
...
function getSpiceCalibration(spiceLevel) {
    if (spiceLevel <= 3) {
...
function tragicRealismBlock() {
    const level = env_1.env.TRAGIC_REALISM_LEVEL ?? 0;
    if (level <= 0)
        return '';
    return `
 ...
 */
function buildPersonPrompt(params) {
    const { system, personName, personData, chartData, spiceLevel, style, personalContext, outputLanguage = languages_1.DEFAULT_OUTPUT_LANGUAGE } = params;
    const systemName = exports.SYSTEM_DISPLAY_NAMES[system];
    const intensity = getProvocationIntensity(spiceLevel);
    // Get language instruction (empty for English)
    const languageInstruction = (0, languages_1.getLanguageInstruction)(outputLanguage);
    return `
 ...
 */
function buildOverlayPrompt(params) {
    const { system, person1Name, person2Name, chartData, spiceLevel, style, relationshipContext, outputLanguage = languages_1.DEFAULT_OUTPUT_LANGUAGE } = params;
    const systemName = exports.SYSTEM_DISPLAY_NAMES[system];
    // Get language instruction (empty for English)
    const languageInstruction = (0, languages_1.getLanguageInstruction)(outputLanguage);
    return `
...
${output_rules_1.RELATIONSHIP_STATUS_RULES}
...
function buildVerdictPrompt(params) {
    const { person1Name, person2Name, allReadingsSummary, person1Triggers = [], person2Triggers = [], overlayTriggers = [], spiceLevel, style, outputLanguage = languages_1.DEFAULT_OUTPUT_LANGUAGE } = params;
    const languageInstruction = (0, languages_1.getLanguageInstruction)(outputLanguage);
    const intensity = getProvocationIntensity(spiceLevel);
    const formatTriggerSection = (title, triggers) => {
        if (!triggers.length)
...
**WORD COUNT: ${wordCounts_1.STANDARD_READING.min}-${wordCounts_1.STANDARD_READING.max} WORDS (target ${wordCounts_1.STANDARD_READING.target}). This becomes ${wordCounts_1.STANDARD_READING.audioMinutes} minutes of audio. DO NOT STOP EARLY.**
...
//# sourceMappingURL=paidReadingPrompts.js.map