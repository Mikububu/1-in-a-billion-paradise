...
exports.NUCLEAR_STRUCTURE = exports.NUCLEAR_PARTS = void 0;
exports.getNuclearPart = getNuclearPart;
exports.buildNuclearStructure = buildNuclearStructure;
exports.buildNuclearPartInstructions = buildNuclearPartInstructions;
exports.NUCLEAR_PARTS = [
    {
        number: 1,
        name: 'Part 1',
...
exports.NUCLEAR_STRUCTURE = {
    name: 'Nuclear Package',
    totalWords: 30000,
    audioMinutes: 150, // 2.5 hours
    parts: exports.NUCLEAR_PARTS,
};
/**
 * Get a specific part configuration
 */
function getNuclearPart(partNumber) {
    return exports.NUCLEAR_PARTS[partNumber - 1];
}
/**
 * Build structure overview for Nuclear reading
 */
function buildNuclearStructure(person1Name, person2Name) {
    const partsOverview = exports.NUCLEAR_PARTS
        .map(p => `**${p.name}: ${p.title}** (${p.words} words)${p.isShadow ? ' ← THE RED ROOM' : ''}
${p.description}`)
        .join('\n\n');
    return `
═══════════════════════════════════════════════════════════════════════════════
STRUCTURE: ${exports.NUCLEAR_STRUCTURE.name}
${person1Name} & ${person2Name} - All 5 Systems Synthesis
═══════════════════════════════════════════════════════════════════════════════

Total: ~${exports.NUCLEAR_STRUCTURE.totalWords} words (~${exports.NUCLEAR_STRUCTURE.audioMinutes} minutes audio)
 ...
 */
function buildNuclearPartInstructions(partNumber, person1Name, person2Name, previousPartSummary) {
 ...
 */
function getPartSpecificInstructions(part, name1, name2) {
    switch (part.number) {
        case 1:
            return `
