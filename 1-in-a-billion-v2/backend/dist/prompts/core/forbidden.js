...
exports.FORBIDDEN_PHRASES_SPICY = exports.FORBIDDEN_PHRASES = void 0;
exports.buildForbiddenSection = buildForbiddenSection;
exports.FORBIDDEN_PHRASES = [
 ...
 */
exports.FORBIDDEN_PHRASES_SPICY = [
 ...
 */
function buildForbiddenSection(style) {
    const base = exports.FORBIDDEN_PHRASES.map(p => `❌ "${p}..."`).join('\n');
    const symbols = `❌ Any decorative symbols / glyphs / emojis (examples: ★ ✦ ✨ • → ✓ ✧ ☉ ॐ). Use plain ASCII punctuation only.`;
    if (style === 'spicy_surreal') {
        const extra = exports.FORBIDDEN_PHRASES_SPICY.map(p => `❌ "${p}" (be direct)`).join('\n');
        return `
