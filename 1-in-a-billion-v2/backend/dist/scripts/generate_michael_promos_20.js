...
function normalizeOutDir(input) {
    const trimmed = String(input || '').trim();
    return trimmed.endsWith('/out')
        ? `${trimmed.replace(/\/out$/, '')}/promo-michael-20`
        : trimmed;
}
const OUT_DIR = normalizeOutDir(process.env.OUT_DIR || '/Users/michaelperinwogenburg/Desktop/1-in-a-billion-media/promo-michael-20');
...
async function generateOne(text, index, take) {
    const base = `${String(index + 1).padStart(2, '0')}_take-${take.name}_michael`;
    const outPath = node_path_1.default.join(OUT_DIR, `${base}.mp3`);
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            node_fs_1.default.writeFileSync(outPath, buf);
            return outPath;
        }
        catch (err) {
            const message = err?.response?.data?.message || err?.message || String(err);
            if (attempt >= 3)
                throw new Error(`${base} failed after 3 attempts: ${message}`);
...
async function main() {
    ...
    for (let i = 0; i < lines.length; i++) {
        for (const take of takes) {
            ...
            outputs.push(file);
            console.log(`✅ ${node_path_1.default.basename(file)}`);
            await sleep(500);
        }
    }
    const durationSec = ((Date.now() - startedAt) / 1000).toFixed(1);
    const manifestPath = node_path_1.default.join(OUT_DIR, 'manifest.json');
    node_fs_1.default.writeFileSync(manifestPath, JSON.stringify({ generatedAt: new Date().toISOString(), durationSec: Number(durationSec), count: outputs.length, files: outputs }, null, 2));
...
//# sourceMappingURL=generate_michael_promos_20.js.map