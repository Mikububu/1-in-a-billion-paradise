...
const DESKTOP = path_2.default.join(HOME, 'Desktop');
const ELISABETH_VOICE_URL = 'https://qdfikbgwuauertfmkmzk.supabase.co/storage/v1/object/public/voices/elisabeth.wav';
...
async function main() {
    ...
    try {
        const st = await promises_1.default.stat(DESKTOP);
        if (!st.isDirectory()) {
    ...
    const michaelAudio = (audioArtifacts || []).find((a) => {
    ...
    if (!michaelAudio) {
        ...
        const recent = (audioArtifacts || []).slice(0, 5).map((a) => a.storage_path);
        if (recent.length) {
            console.error('   Recent artifacts (first 5):');
            recent.forEach((p) => console.error('     -', p));
        }
        process.exit(1);
    }
    ...
    if (!text) {
        ...
        const match = textArtifacts.find((a) => a.metadata?.docNum === docNum ||
    ...
    try {
        ...
        const outPath = path_2.default.join(DESKTOP, outName);
        ...
        const sizeKb = (stat.size || buf.length) / 1024;
        console.log('\n═══════════════════════════════════════════════════════════');
...
main();
//# sourceMappingURL=regenerateMichaelAudioTest.js.map