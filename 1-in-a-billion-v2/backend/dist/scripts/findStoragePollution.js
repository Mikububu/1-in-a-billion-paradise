        ...
        if (!artifactsError && artifacts) {
            ...
            for (const artifact of artifacts) {
                if (artifact.metadata) {
                    ...
                    if (metaSize > largestMetadata)
                        largestMetadata = metaSize;
                }
            }
        ...
        if (!jobsError && jobs) {
            ...
            for (const job of jobs) {
                if (job.params) {
                    ...
                    if (paramsSize > largestParams)
                        largestParams = paramsSize;
                }
                if (job.progress) {
                    const progressSize = JSON.stringify(job.progress).length;
                    totalProgressSize += progressSize;
                    if (progressSize > largestProgress)
                        largestProgress = progressSize;
                }
            }
        ...
        if (!tasksError && tasks) {
            ...
            for (const task of tasks) {
                if (task.input) {
                    ...
                    if (inputSize > largestInput)
                        largestInput = inputSize;
                }
                if (task.output) {
                    const outputSize = JSON.stringify(task.output).length;
                    totalOutputSize += outputSize;
                    if (outputSize > largestOutput)
                        largestOutput = outputSize;
                }
            }
...
//# sourceMappingURL=findStoragePollution.js.map