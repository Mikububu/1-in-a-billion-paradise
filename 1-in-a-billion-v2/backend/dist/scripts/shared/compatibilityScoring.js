...
exports.generateCompatibilityScores = generateCompatibilityScores;
...
function buildScoringPrompt(params) {
    ...
    const categoryBlock = categories.map((cat) => `${cat.key}: [0-100]\n[2-3 sentences: ${cat.instruction}]`).join('\n\n');
    return [
...
function parseScores(raw) {
    ...
    while ((m = scoreBlockRe.exec(text)) !== null) {
        ...
        if (!rawLabel || !Number.isFinite(score100))
            continue;
        if (/^(COMPATIBILITY|SCORING|FORMAT|OUTPUT|STRUCTURE|STYLE|BASED|SCORE)/i.test(rawLabel))
            continue;
...
async function generateCompatibilityScores(params) {
    ...
    const raw = await llm_1.llmPaid.generateStreaming(prompt, `${label}:scoring`, {
        provider: 'claude',
        maxTokens: 4096,
        temperature: 0.3,
        maxRetries: 5,
