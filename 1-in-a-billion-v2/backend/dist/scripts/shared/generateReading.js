...
exports.safeFileToken = safeFileToken;
exports.generateSingleReading = generateSingleReading;
...
const verdictTrigger_1 = require("../../promptEngine/triggerEngine/verdictTrigger");
const triggerConfig_1 = require("../../promptEngine/triggerEngine/triggerConfig");
const CLAUDE_MAX_TOKENS_PER_CALL = 16384;
const SCRIPT_VERDICT_POLICY = 'script_reconstruct';
let repoSafetyChecked = false;
function assertSafeRepoStateForV2() {
    if (repoSafetyChecked)
        return;
    repoSafetyChecked = true;
    // Escape hatch for local emergency runs.
    if (process.env.ALLOW_UNSAFE_ROOT === '1')
        return;
    const v2Root = node_path_1.default.resolve(__dirname, '../../../..');
    let status = '';
    try {
        status = (0, node_child_process_1.execSync)(`git -C "${v2Root}" status --porcelain`, {
            encoding: 'utf8',
...
function isIncarnationStyle(styleLayerId) {
    return safeFileToken(styleLayerId).includes('incarnation');
}
function isSoulMemoirStyle(styleLayerId) {
...
function isHeadlineLine(line) {
    ...
    if (upperWords / words.length >= 0.9)
        return true;
    if (/^(?:THE|The|A|An)\s+/.test(normalized) && words.length >= 4)
        return true;
    const titleLikeWords = words.filter((w) => /^[A-Z][A-Za-z'’\-]*$/.test(w) || /^[A-Z0-9'’\-]+$/.test(w)).length;
    return titleLikeWords / words.length >= 0.85;
}
function splitInlineAllCapsHeadlines(text) {
...
function splitInlineRomanHeadings(text) {
    const src = String(text || '');
...
function capSurrealHeadlines(text, maxHeadlines = 6) {
...
function ensureSectionHeadlines(text, minHeadlines = 4, maxHeadlines = 6) {
    const raw = String(text || '').trim();
    if (!raw)
        return raw;
...
function stripLeadingCoverMetadata(text) {
    const lines = String(text || '').split('\n');
    let i = 0;
    const isDateLine = (line) => /^(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}$/i.test(line.trim());
    const isBirthLine = (line) => /\b\d{1,2}:\d{2}\b/.test(line) && /·/.test(line);
    const isTitleLine = (line) => /^(Western Astrology|Vedic Astrology|Human Design|Gene Keys|Kabbalah)\b/i.test(line.trim());
    const isNameOnly = (line) => /^[A-Z][A-Za-z' -]{1,80}$/.test(line.trim());
    if (i < lines.length && isTitleLine(lines[i] || ''))
        i += 1;
    if (i < lines.length && isDateLine(lines[i] || ''))
        i += 1;
    if (i < lines.length && isNameOnly(lines[i] || ''))
        i += 1;
    if (i < lines.length && isBirthLine(lines[i] || ''))
        i += 1;
    if (i > 0) {
...
function tightenParagraphs(raw) {
...
function stripHouseDefinitionSentences(text) {
    const houseDef = /\bthe\s+(?:first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|eleventh|twelfth|\d+(?:st|nd|rd|th))\s+house\s+is\b[^.!?]*[.!?]?/gi;
    return String(text || '')
        .replace(houseDef, ' ')
        .replace(/[ \t]{2,}/g, ' ')
        .replace(/\n[ \t]+/g, '\n')
...
function normalizePronounGrammar(text) {
    let out = String(text || '');
...
function findPronounGrammarIssue(text) {
    const body = String(text || '');
    ...
    if (new RegExp(`\\b${preposition}\\s+he\\b`, 'i').test(body))
        return 'he_after_preposition';
    if (new RegExp(`\\b${preposition}\\s+she\\b`, 'i').test(body))
        return 'she_after_preposition';
    if (/\bwhat have he\b/i.test(body))
        return 'what_have_he';
    if (/\bwhat have she\b/i.test(body))
        return 'what_have_she';
    if (/\bwhere he stand\b/i.test(body))
        return 'where_he_stand';
    if (/\bwhere she stand\b/i.test(body))
        return 'where_she_stand';
    if (/\bwhat he do\b/i.test(body))
        return 'what_he_do';
    if (/\bwhat she do\b/i.test(body))
        return 'what_she_do';
    return null;
}
...
function getComplianceIssues(text) {
    ...
    if (hasSecondPerson(text))
        issues.push({ kind: 'second_person' });
    const forbiddenIds = getForbiddenMatchIds(text);
    if (forbiddenIds.length > 0)
        issues.push({ kind: 'forbidden_phrase', ids: forbiddenIds });
    if (hasBannedDetours(text))
        issues.push({ kind: 'banned_detour' });
    return issues;
}
...
function extractZone2Text(text, zone1WordBudget = 600) {
    const raw = String(text || '').trim();
    if (!raw)
        return '';
    const re = /\S+/g;
...
function getEvidenceAnchors(system, styleLayerId) {
    if (isIncarnationStyle(styleLayerId))
        return 'Translate chart mechanics into mythology, weather, architecture, and behavior. Avoid technical placement syntax and report-like exposition.';
...
function normalizeSoulMemoirFooter(text) {
    const raw = String(text || '');
    const lines = raw.split('\n');
    let chartSignatureLine;
    let dataLine;
    const kept = [];
    ...
    const footer = [];
    if (chartSignatureLine)
        footer.push(chartSignatureLine);
    if (dataLine)
        footer.push(dataLine);
    if (footer.length === 0)
        return body;
    return `${body}\n\n${footer.join('\n')}`.trim();
}
function validateSoulMemoirContract(text) {
    const raw = String(text || '');
...
async function repairComplianceIfNeeded(options) {
    ...
    for (let pass = 1; pass <= MAX_PASSES; pass += 1) {
        ...
        const repaired = await llm_1.llmPaid.generateStreaming(repairPrompt, `${options.label}:repair:${pass}`, {
            provider: 'claude',
...
async function rewriteIncarnationStyleIfNeeded(options) {
    ...
    for (let pass = 1; pass <= MAX_REWRITE_PASSES; pass += 1) {
        ...
        if (zone2Issues.length === 0 && technicalLines.length === 0 && !ageMismatch && !fullSecondPersonLeak)
            return out;
        ...
        if (minorTechnicalDrift && nonSecondIssues.length === 0 && !fullSecondPersonLeak) {
            ...
            if (postIssues.length === 0 && postTechnical.length === 0 && !postAgeMismatch && !postSecondPerson && !postPronounIssue)
                return cleaned;
            out = cleaned;
            continue;
        }
        ...
        const rewritten = await llm_1.llmPaid.generateStreaming(rewritePrompt, `${options.label}:incarnation-rewrite:${pass}`, {
            provider: 'claude',
            maxTokens: CLAUDE_MAX_TOKENS_PER_CALL,
            temperature: 0.6,
            maxRetries: 5,
...
async function expandToHardFloor(options) {
    ...
    for (let pass = 1; pass <= MAX_PASSES; pass += 1) {
        ...
        if (currentWords >= options.hardFloorWords)
            break;
        ...
        const chunk = await llm_1.llmPaid.generateStreaming(expansionPrompt, `${options.label}:expand:${pass}`, {
            provider: 'claude',
            maxTokens: CLAUDE_MAX_TOKENS_PER_CALL,
            temperature: 0.8,
            maxRetries: 5,
...
function countHeadlineLines(text) {
...
function stripControlTextLeaks(raw) {
    const escapedTriggerTitle = triggerConfig_1.NARRATIVE_TRIGGER_TITLE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const escapedRelationalTriggerTitle = triggerConfig_1.RELATIONAL_TRIGGER_TITLE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const lineKillRe = new RegExp(`\\b(?:before writing|internal arc|NARRATIVE_ARC|THE_${escapedTriggerTitle.replace(/ /g, '\\s+')}|THE_DEFENSE|ACT_1_SURFACE|ACT_2_BENEATH|ACT_3_RECKONING|LANDING_TEMPERATURE|${escapedRelationalTriggerTitle.replace(/ /g, '\\s+')}|FINAL OUTPUT REQUIREMENT|OUTPUT CONTRACT|ZONE 1|ZONE 2|NARRATOR:|METAPHOR WORLD:|STRUCTURE:|ANTI-SURVEY:|LENGTH:|CHART DATA(?:\\s*\\(.*\\))?:)\\b`, 'i');
...
function buildTriggerPipelineBundle(options) {
    ...
    if (docType === 'verdict' || payloadBase?.type === 'bundle_verdict') {
        ...
        if (verdictMode !== SCRIPT_VERDICT_POLICY) {
            throw new Error(`Unsupported verdictMode "${verdictMode}" for script path. Expected "${SCRIPT_VERDICT_POLICY}".`);
        }
        const stripped = (0, verdictTrigger_1.stripVerdictChartData)(chartData);
        return {
            strippedChartData: stripped,
            triggerPrompt: (0, verdictTrigger_1.buildVerdictTriggerPrompt)({
                person1Name: p1Name,
                person2Name: p2Name,
                strippedChartData: stripped,
            }),
            writingPromptBase: (0, verdictTrigger_1.buildVerdictWritingPrompt)({
                person1Name: p1Name,
                person2Name: p2Name,
    ...
    if (docType === 'individual') {
        const stripBuilders = {
            western: westernTrigger_1.stripWesternChartData,
            vedic: vedicTrigger_1.stripVedicChartData,
            human_design: humanDesignTrigger_1.stripHDChartData,
            gene_keys: geneKeysTrigger_1.stripGeneKeysChartData,
            kabbalah: kabbalahTrigger_1.stripKabbalahChartData,
        };
        const triggerBuilders = {
            western: westernTrigger_1.buildWesternTriggerPrompt,
            vedic: vedicTrigger_1.buildVedicTriggerPrompt,
            human_design: humanDesignTrigger_1.buildHDTriggerPrompt,
            gene_keys: geneKeysTrigger_1.buildGeneKeysTriggerPrompt,
            kabbalah: kabbalahTrigger_1.buildKabbalahTriggerPrompt,
        };
        const writingBuilders = {
            western: westernTrigger_1.buildWesternWritingPrompt,
            vedic: vedicTrigger_1.buildVedicWritingPrompt,
            human_design: humanDesignTrigger_1.buildHDWritingPrompt,
            gene_keys: geneKeysTrigger_1.buildGeneKeysWritingPrompt,
            kabbalah: kabbalahTrigger_1.buildKabbalahWritingPrompt,
        };
        const buildTrigger = triggerBuilders[system];
        const buildWriting = writingBuilders[system];
        const strip = stripBuilders[system];
        if (!strip || !buildTrigger || !buildWriting)
            return null;
    ...
    if (!p1Raw || !p2Raw)
        return null;
    const stripOverlayBuilders = {
        western: overlayTrigger_1.stripWesternOverlayData,
        vedic: overlayTrigger_1.stripVedicOverlayData,
        human_design: overlayTrigger_1.stripHDOverlayData,
        gene_keys: overlayTrigger_1.stripGeneKeysOverlayData,
        kabbalah: overlayTrigger_1.stripKabbalahOverlayData,
    };
    const overlayTriggerBuilders = {
        western: overlayTrigger_1.buildWesternOverlayTriggerPrompt,
        vedic: overlayTrigger_1.buildVedicOverlayTriggerPrompt,
        human_design: overlayTrigger_1.buildHDOverlayTriggerPrompt,
        gene_keys: overlayTrigger_1.buildGeneKeysOverlayTriggerPrompt,
        kabbalah: overlayTrigger_1.buildKabbalahOverlayTriggerPrompt,
    };
    const overlayWritingBuilders = {
        western: overlayTrigger_1.buildWesternOverlayWritingPrompt,
        vedic: overlayTrigger_1.buildVedicOverlayWritingPrompt,
        human_design: overlayTrigger_1.buildHDOverlayWritingPrompt,
        gene_keys: overlayTrigger_1.buildGeneKeysOverlayWritingPrompt,
        kabbalah: overlayTrigger_1.buildKabbalahOverlayWritingPrompt,
    };
    ...
    if (!stripOverlay || !buildOverlayTrigger || !buildOverlayWriting)
        return null;
...
async function generateSingleReading(options) {
    assertSafeRepoStateForV2();
    const { system, personName, styleLayerId, outDir, fileBase, chartData, chartDataPerson1, chartDataPerson2, payloadBase, hardFloorWords, docType, } = options;
    ...
    const triggerPromptPath = writeDebugArtifacts ? node_path_1.default.join(outDir, `${fileBase}.trigger.prompt.txt`) : '';
    const writingPromptPath = writeDebugArtifacts ? node_path_1.default.join(outDir, `${fileBase}.writing.prompt.txt`) : '';
    const systemPromptPath = writeDebugArtifacts ? node_path_1.default.join(outDir, `${fileBase}.system.txt`) : '';
    const readingPath = writeDebugArtifacts ? node_path_1.default.join(outDir, `${fileBase}.reading.txt`) : '';
    if (writeDebugArtifacts) {
        node_fs_1.default.writeFileSync(triggerPromptPath, triggerBundle.triggerPrompt, 'utf8');
        node_fs_1.default.writeFileSync(systemPromptPath, '', 'utf8');
    }
    const triggerRaw = await llm_1.llmPaid.generateStreaming(triggerBundle.triggerPrompt, `${fileBase}:trigger`, {
        provider: 'claude',
        maxTokens: 1200,
        temperature: 0.45,
        maxRetries: 5,
    ...
    if (!narrativeTrigger)
        throw new Error(`Trigger call returned empty text for ${fileBase}`);
    ...
    const chartProvocations = docType === 'individual'
        ? (0, chartProvocations_1.buildChartAwareProvocations)(personName, system, chartData, spiceLevel)
        : ''; // Overlay provocations not needed (chart data already has both charts)
    // FIX 2: Style-specific system prompt
    const llmSystemPrompt = (0, styles_1.getSystemPromptForStyle)('spicy_surreal', docType === 'overlay' ? 'overlay' : 'individual');
    ...
    if (writeDebugArtifacts) {
        node_fs_1.default.writeFileSync(writingPromptPath, writingPrompt, 'utf8');
        node_fs_1.default.writeFileSync(systemPromptPath, llmSystemPrompt, 'utf8');
    }
    let finalReading = '';
    const MAX_ATTEMPTS = 2;
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
        const raw = await llm_1.llmPaid.generateStreaming(writingPrompt, `${fileBase}:writing:${attempt}`, {
            provider: 'claude',
            maxTokens: CLAUDE_MAX_TOKENS_PER_CALL,
            temperature: 0.8,
            maxRetries: 5,
        ...
        const escapedTriggerTitle = triggerConfig_1.NARRATIVE_TRIGGER_TITLE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const escapedRelationalTriggerTitle = triggerConfig_1.RELATIONAL_TRIGGER_TITLE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const controlLeakRe = new RegExp(`\\b(?:THE_${escapedTriggerTitle.replace(/ /g, '\\s+')}|before writing|internal arc|NARRATIVE_ARC|ACT_1_SURFACE|ACT_2_BENEATH|ACT_3_RECKONING|LANDING_TEMPERATURE|${escapedRelationalTriggerTitle.replace(/ /g, '\\s+')}|OUTPUT CONTRACT|FINAL OUTPUT REQUIREMENT|CHART DATA)\\b`, 'i');
    ...
    if (writeDebugArtifacts) {
        node_fs_1.default.writeFileSync(readingPath, finalReading, 'utf8');
    }
    return {
...
//# sourceMappingURL=generateReading.js.map