 ...
 */
async function purgeUserById(serviceClient, userId) {
    ...
    try {
        const { data: vedicJobs } = await serviceClient
            ...
            .select('id')
            .eq('user_id', userId);
        const ids = Array.isArray(vedicJobs) ? vedicJobs.map((j) => j.id) : [];
        if (ids.length) {
    ...
    try {
        const { data: audiobookJobs } = await serviceClient
            ...
            .select('id')
            .eq('user_id', userId);
        const jobIds = Array.isArray(audiobookJobs) ? audiobookJobs.map((j) => j.id) : [];
        if (jobIds.length) {
    ...
    try {
        ...
        if (names.length) {
            const filePaths = names.map((n) => `${userId}/${n}`);
...
async function listAllUserIds(serviceClient) {
    const perPage = 200;
    let page = 1;
    const ids = [];
    for (;;) {
...
//# sourceMappingURL=purgeUsers.js.map