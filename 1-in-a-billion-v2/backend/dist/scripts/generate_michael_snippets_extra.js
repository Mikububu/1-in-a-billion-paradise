...
const OUT_DIR = normalizeOutDir(process.env.OUT_DIR || '/Users/michaelperinwogenburg/Desktop/1-in-a-billion-media/promo-michael-extra');
...
async function render(text, index) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            node_fs_1.default.writeFileSync(outPath, Buffer.from(res.data.audioBase64, 'base64'));
            return outPath;
        }
        catch (err) {
            const msg = err?.response?.data?.message || err?.message || String(err);
            if (attempt >= 3)
                throw new Error(`${base} failed after 3 attempts: ${msg}`);
...
async function main() {
    ...
    for (let i = 0; i < snippets.length; i++) {
        ...
        files.push(out);
        console.log(`✅ ${node_path_1.default.basename(out)} (${i + 1}/${snippets.length})`);
        await sleep(400);
    }
...
//# sourceMappingURL=generate_michael_snippets_extra.js.map