...
const BASE_URL = `http://localhost:${env_1.env.PORT || 8787}`;
const results = [];
...
async function testVedicHealth() {
    await test('Vedic Health Check (v1)', async () => {
        const response = await axios_1.default.get(`${BASE_URL}/api/vedic/health`, { timeout: 5000 });
...
async function testVedicV2Health() {
    await test('Vedic V2 Health Check', async () => {
        const response = await axios_1.default.get(`${BASE_URL}/api/vedic-v2/health`, { timeout: 5000 });
        if (response.status !== 200)
            throw new Error(`Status ${response.status}`);
        return response.data;
    });
...
async function testVedicMatch() {
    await test('Vedic Match Endpoint', async () => {
        ...
        try {
            const response = await axios_1.default.post(`${BASE_URL}/api/vedic/match`, testData, { timeout: 10000 });
            if (response.status !== 200)
                throw new Error(`Status ${response.status}`);
...
async function testVedicScore() {
    await test('Vedic Score Endpoint', async () => {
        ...
        try {
            const response = await axios_1.default.post(`${BASE_URL}/api/vedic/score`, testData, { timeout: 10000 });
            if (response.status !== 200)
                throw new Error(`Status ${response.status}`);
...
async function testVedicV2Match() {
    await test('Vedic V2 Match Endpoint', async () => {
        ...
        try {
            const response = await axios_1.default.post(`${BASE_URL}/api/vedic-v2/match`, testData, { timeout: 10000 });
            if (response.status !== 200)
                throw new Error(`Status ${response.status}`);
            return { status: response.status, hasData: !!response.data };
        }
...
//# sourceMappingURL=testVedicModules.js.map