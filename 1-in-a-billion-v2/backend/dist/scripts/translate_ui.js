...
const I18N_DIR = path_1.default.join(__dirname, '../../../src/i18n');
const EN_PATH = path_1.default.join(I18N_DIR, 'en.json');
const LANGUAGES = [
...
async function translateBatch(keys, enDict, targetLang) {
    const sourceObj = {};
    for (const k of keys) {
...
`;
    try {
        const { text } = await llm_1.llmWithFallback.generateWithFallback(prompt, `translate-${targetLang}`, { maxTokens: 8192 });
...
async function main() {
    const enDict = JSON.parse(fs_1.default.readFileSync(EN_PATH, 'utf-8'));
    const allKeys = Object.keys(enDict);
    for (const lang of LANGUAGES) {
        console.log(`\n🌍 Translating to ${lang.name} (${lang.code})...`);
        const outPath = path_1.default.join(I18N_DIR, `${lang.code}.json`);
        // Load existing if any, to skip already translated
        let resultDict = {};
        if (fs_1.default.existsSync(outPath)) {
            try {
                const existing = JSON.parse(fs_1.default.readFileSync(outPath, 'utf-8'));
        ...
        for (let i = 0; i < missingKeys.length; i += BATCH_SIZE) {
            ...
            while (!success && retries > 0) {
                try {
                    ...
                    success = true;
                    // Save incrementally
                    fs_1.default.writeFileSync(outPath, JSON.stringify(resultDict, null, 2), 'utf-8');
                }
                catch (err) {
                    retries--;
