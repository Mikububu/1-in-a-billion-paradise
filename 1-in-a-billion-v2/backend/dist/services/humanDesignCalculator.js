...
exports.HD_CHANNELS = void 0;
exports.calculateHumanDesign = calculateHumanDesign;
 ...
 */
function determineType(definedCenters, activeChannels) {
    ...
    if (hasSacral) {
        ...
        const hasMotorToThroat = activeChannels.some(channelId => {
            const channel = exports.HD_CHANNELS.find(c => c.id === channelId);
            if (!channel)
                return false;
            // Check if this channel connects a motor center to Throat
            const connectsMotorToThroat = (MOTOR_CENTERS.includes(channel.center1) && channel.center2 === 'Throat') ||
 ...
 */
function calculateProfile(personalitySun, designSun) {
 ...
 */
function calculateIncarnationCross(personalitySun, personalityEarth, designSun, designEarth) {
 ...
 */
function calculateHumanDesign(positions) {
    const getGate = (longitude) => {
        if (typeof longitude !== 'number')
            return { gate: 0, line: 0 };
        const gk = (0, geneKeysCalculator_1.tropicalLongitudeToGeneKey)(longitude);
    ...
    Object.values(personality).forEach(p => { if (p.gate > 0)
        allGates.add(p.gate); });
    Object.values(design).forEach(p => { if (p.gate > 0)
        allGates.add(p.gate); });
    const activeGates = Array.from(allGates);
    // Identify defined channels and centers
    const activeChannels = [];
    const definedCenters = new Set();
    exports.HD_CHANNELS.forEach(channel => {
    ...
    const incarnationCross = calculateIncarnationCross(personality.sun, personality.earth, design.sun, design.earth);
    return {
        personality,
        design,
        activeGates,
...
//# sourceMappingURL=humanDesignCalculator.js.map