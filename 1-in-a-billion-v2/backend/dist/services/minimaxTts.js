...
exports.getMinimaxSequenceForUrl = getMinimaxSequenceForUrl;
exports.generateMinimaxAsync = generateMinimaxAsync;
exports.registerVoiceClone = registerVoiceClone;
exports.uploadForVoiceClone = uploadForVoiceClone;
const axios_1 = __importDefault(require("axios"));
const form_data_1 = __importDefault(require("form-data"));
const tar = __importStar(require("tar-stream"));
const apiKeysHelper_1 = require("./apiKeysHelper");
 ...
 */
async function uploadReferenceAudio(wavBuffer, filename = 'reference.wav', purpose = 'prompt_audio') {
    const key = await apiKeysHelper_1.apiKeys.minimax().catch(() => null) || env_1.env.MINIMAX_API_KEY;
    if (!key)
    ...
    const upRes = await axios_1.default.post('https://api.minimax.io/v1/files/upload', form, {
    ...
    if (!fileId)
        throw new Error("MiniMax did not return a file_id after upload");
 ...
 */
async function getMinimaxSequenceForUrl(url, filename) {
 ...
 */
async function generateMinimaxAsync(text, voiceId, clonePromptFileId, speed = 1.0, volume = 1.0, language) {
    try {
        const apiKey = await apiKeysHelper_1.apiKeys.minimax().catch(() => null) || env_1.env.MINIMAX_API_KEY;
        if (!apiKey)
            throw new Error("MiniMax API key not found");
        ...
        if (clonePromptFileId) {
            ...
            payload.clone_prompt = {
                ...
                prompt_text: voices_1.VOICE_CLONE_TRANSCRIPT,
            };
            console.log(`[MiniMax TTS] Clone prompt set: audio=${clonePromptFileId}, transcript=${voices_1.VOICE_CLONE_TRANSCRIPT.substring(0, 60)}...`);
        }
        const submitRes = await axios_1.default.post('https://api.minimax.io/v1/t2a_async_v2', payload, {
        ...
        while (true) {
            ...
            const fetchRes = await axios_1.default.get(`https://api.minimax.io/v1/query/t2a_async_query_v2?task_id=${taskId}`, {
            ...
            if (status === 'Success') {
                ...
                const retrieveRes = await axios_1.default.get(`https://api.minimax.io/v1/files/retrieve?file_id=${outputFileId}`, {
                    headers: { 'Authorization': `Bearer ${apiKey}` }
                });
                const downloadUrl = retrieveRes.data.file?.download_url;
                if (!downloadUrl)
                    throw new Error('MiniMax did not return a download_url for the generated audio');
 ...
 */
async function registerVoiceClone(audioFileId, voiceId, model = 'speech-01-turbo') {
    const key = await apiKeysHelper_1.apiKeys.minimax().catch(() => null) || env_1.env.MINIMAX_API_KEY;
    if (!key)
        throw new Error("MiniMax API key not found");
    ...
    const res = await axios_1.default.post('https://api.minimax.io/v1/voice_clone', {
        file_id: audioFileId,
        voice_id: voiceId,
        model,
 ...
 */
async function uploadForVoiceClone(wavBuffer, filename = 'reference.wav') {
    return uploadReferenceAudio(wavBuffer, filename, 'voice_clone');
}
async function downloadAndExtractMp3FromTar(url) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            extract.on('finish', () => {
                if (!found)
                    reject(new Error('MP3 file not found inside the MiniMax output tar stream'));
            });
...
//# sourceMappingURL=minimaxTts.js.map