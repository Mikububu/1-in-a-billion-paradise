...
exports.buildChartDataForSystem = buildChartDataForSystem;
function buildChartDataForSystem(system, person1Name, p1Placements, person2Name, p2Placements, p1BirthData, p2BirthData) {
    const formatDegree = (d) => (d ? `${d.degree}° ${d.minute}'` : '');
    ...
    const fallbackLongitude = (pl, which) => {
        const sign = which === 'sun'
    ...
    const p1SunDeg = typeof p1Placements?.sunLongitude === 'number'
        ? p1Placements.sunLongitude
        : fallbackLongitude(p1Placements, 'sun');
    const p2SunDeg = hasP2 && typeof p2Placements?.sunLongitude === 'number'
        ? p2Placements.sunLongitude
        : hasP2
    ...
    const getAgeYears = (birthDate, now) => {
    ...
    const buildWesternSection = (name, placements, birthData) => {
        ...
        const mc = fmtDegObj(typeof t?.mcLongitude === 'number' ? toDegreesObj(t.mcLongitude) : undefined);
        const sun = t.planets.find((p) => p.key === 'sun');
        const sect = typeof sun?.house === 'number'
            ? sun.house >= 7
                ? 'Day'
                : 'Night'
            : undefined;
        ...
        const cuspLines = (t.houseCusps || [])
            .slice(0, 12)
            .map((c, idx) => `- ${idx + 1}: ${fmtDegObj(toDegreesObj(c))}`)
            .join('\n');
        ...
        const aspectLines = (t.aspects || [])
            .map((a) => `- ${a.a} ${String(a.type).toUpperCase()} ${a.b} (orb ${a.orb}°${a.exact ? ', exact' : ''})`)
            .join('\n');
        ...
        const transitAspectLines = (t.transits?.aspectsToNatal || [])
            .slice(0, 40)
            .map((a) => `- ${a.transit} ${String(a.type).toUpperCase()} ${a.natal} (orb ${a.orb}°${a.exact ? ', exact' : ''})`)
            .join('\n');
        const profectionBlock = typeof age === 'number' && profSign
            ? [
    ...
    function toDegreesObj(longitude) {
    ...
    switch (system) {
        ...
        case 'vedic': {
            ...
            const getVedicDignity = (grahaKey, sign) => {
                if (exaltation[grahaKey] === sign)
                    return 'uchcha';
                if (debilitation[grahaKey] === sign)
                    return 'neecha';
                if (moolatrikona[grahaKey] === sign)
                    return 'moolatrikona';
            ...
            const buildVedicBlock = (name, placements, birthData) => {
                ...
                const now = new Date();
                const age = getAgeYears(birthData.birthDate, now);
                const grahas = (sid.grahas || []).filter((g) => !g.isTrueNode);
                const grahaLines = grahas.map((g) => {
                ...
                const lagnaLordGraha = grahas.find((g) => g.key === lagnaLordKey);
                ...
                const lord7graha = grahas.find((g) => g.key === lord7key);
                ...
                return [
                    ...
                    (() => {
                        ...
                        if (!d)
                            return 'VIMSHOTTARI DASHA: Not computed.';
                        const maha = d.mahadasha;
                        const antar = d.antardasha;
                        const sequence = (d.allMahadashas || [])
                            .map((m) => `  ${m.isCurrent ? '▶' : ' '} ${m.lord} (${m.years}yr) ${m.startDate} → ${m.endDate}`)
                            .join('\n');
                        return [
                            `VIMSHOTTARI DASHA:`,
                    ...
                    (() => {
                        ...
                        if (!nav)
                            return 'NAVAMSHA (D-9): Not computed.';
                        const grahaLines = (nav.grahas || [])
                            .map((g) => `- ${grahaDisplayName[g.key] || g.key}: ${g.navamshaSign}`)
                            .join('\n');
                        return [
            ...
            try {
                ...
                if (p1Sid?.janmaNakshatra && p2Sid?.janmaNakshatra && p1Sid?.chandraRashi && p2Sid?.chandraRashi) {
                    ...
                    const moonGraha1 = (p1Sid.grahas || []).find((g) => g.key === 'moon');
                    const moonGraha2 = (p2Sid.grahas || []).find((g) => g.key === 'moon');
                    const marsGraha1 = (p1Sid.grahas || []).find((g) => g.key === 'mars');
                    const marsGraha2 = (p2Sid.grahas || []).find((g) => g.key === 'mars');
                    const chart1 = {
                        id: 'person1',
                    ...
                    if (total >= 28)
                        tier = 'Excellent (28+ - highly auspicious)';
                    else if (total >= 24)
                        tier = 'Very Good (24-27 - strongly recommended)';
                    else if (total >= 18)
                        tier = 'Good (18-23 - acceptable)';
                    // Dosha flags
                    const doshaLines = [];
                    if (a.nadi.dosha_present)
                        doshaLines.push('⚠ NADI DOSHA: Same Nadi - traditionally considered a serious obstruction (health/progeny concerns). Score: 0/8');
                    if (a.bhakoot.score === 0)
                        doshaLines.push('⚠ BHAKOOT DOSHA: Inauspicious Moon sign combination - may indicate financial or emotional friction. Score: 0/7');
        ...
        case 'human_design': {
            ...
            const buildHDBlock = (name, placements) => {
                ...
                return [
                    ...
                    channelLines,
                    '',
                    `ALL ACTIVE GATES: ${(hd.activeGates || []).sort((a, b) => a - b).join(', ') || 'Unknown'}`,
                    '',
            ...
            if (!hasP2)
                return buildHDBlock(person1Name, p1Placements);
        ...
        case 'gene_keys': {
            ...
            if (!hasP2)
                return buildGeneKeysBlock(person1Name, p1GK);
            return [buildGeneKeysBlock(person1Name, p1GK), '', buildGeneKeysBlock(person2Name, p2GK)].join('\n');
        }
        case 'kabbalah': {
            const buildKabbalahBlock = (name, placements, birthData) => {
                ...
                const strong = (profile.sefiroticProfile?.dominant || []).filter((d) => d.strength === 'strong');
                const moderate = (profile.sefiroticProfile?.dominant || []).filter((d) => d.strength === 'moderate');
                const weak = (profile.sefiroticProfile?.dominant || []).filter((d) => d.strength === 'weak');
                ...
                const lsLines = ls
                    ? [
                        ...
                        `- Active Letters: ${(ls.allActiveLetters || []).map((x) => `${x.letter}(${x.name})[${x.type}:${String(x.planet).toUpperCase()}]`).join(', ') || 'none'}`,
                    ].join('\n')
                ...
                const kpLines = kp
                    ? [
                        ...
                        `- Active Klipoth: ${(kp.activeKlipoth || []).map((k) => `${k.sefirah}:${k.klipahName} (trigger=${k.trigger})`).join(' | ') || 'none'}`,
                        `- Hard Aspects: ${(kp.hardAspects || []).map((a) => `${a.planet1} ${a.type} ${a.planet2} (orb ${a.orb})`).join(' | ') || 'none'}`,
                    ].join('\n')
                    : '- Klipoth: Unknown';
                const transitWeather = (() => {
                    ...
                    const planetToSefirah = new Map();
                    for (const d of profile.sefiroticProfile?.dominant || []) {
                        if (d?.planet && d?.sefirah)
                            planetToSefirah.set(String(d.planet).toLowerCase(), String(d.sefirah));
                    }
                    const lines = aspectsToNatal
                        .filter((a) => typeof a?.transit === 'string' && typeof a?.natal === 'string')
                        .filter((a) => a.transit.startsWith('T_'))
                        .filter((a) => classical.has(String(a.natal).toUpperCase()) && classical.has(String(a.transit).slice(2).toUpperCase()))
...
//# sourceMappingURL=chartDataBuilder.js.map