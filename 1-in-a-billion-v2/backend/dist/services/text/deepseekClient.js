...
${(env_1.env.TRAGIC_REALISM_LEVEL ?? 0) > 0 ? `
...
`.trim();
        ...
        const { llm } = await Promise.resolve().then(() => __importStar(require('../llm')));
        try {
            ...
            return { reading: { content: cleanText(content) }, source: 'deepseek' };
        }
        catch (error) {
...
`.trim();
        try {
            ...
            const rawContent = await llm_1.llm.generate(fullPrompt, `synastry-${system}`, {
                maxTokens: 3000,
                temperature: 0.8,
            });
            const normalized = rawContent.replace(/```json|```/g, '').trim();
