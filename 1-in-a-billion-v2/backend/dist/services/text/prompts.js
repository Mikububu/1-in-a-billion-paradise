...
exports.buildReadingPrompt = buildReadingPrompt;
const env_1 = require("../../config/env");
function formatBirthDate(dateStr, language = 'en') {
...
TRAGIC REALISM (LEVEL ${env_1.env.TRAGIC_REALISM_LEVEL}):
...
function getDecanDescription(degree, decan) {
    if (!decan) {
        if (degree < 10)
            return 'where the sign is still forming itself, at its very point of origin';
        if (degree < 20)
            return 'where the sign reaches its fullest expression';
...
function formatHouse(house, language) {
    if (!house)
        return 'house position unknown';
...
function buildReadingPrompt(ctx) {
    ...
    const partnerPlacementExplanations = {
...
${env_1.env.TRAGIC_REALISM_LEVEL >= 1 ? `
TRAGIC REALISM (LEVEL ${env_1.env.TRAGIC_REALISM_LEVEL}) - FIT THIS INTO THE WORD LIMIT:
