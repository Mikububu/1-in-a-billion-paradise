 ...
 */
function loadMusicPrompt(system) {
    const systemMap = {
    ...
    const promptPath = path_1.default.join(process.cwd(), 'prompts/music', fileName);
    try {
        if (fs_1.default.existsSync(promptPath)) {
            const promptTemplate = fs_1.default.readFileSync(promptPath, 'utf-8');
            ...
            return fs_1.default.readFileSync(path_1.default.join(process.cwd(), 'prompts/music/western-music-prompt.md'), 'utf-8');
        }
    }
    catch (err) {
...
async function generateLyrics(input) {
    ...
    const normalizeSystem = () => {
        if (systemKey && allowedSystems.includes(systemKey))
            return systemKey;
        ...
        if (raw.includes('human design'))
            return 'human_design';
        if (raw.includes('gene keys'))
            return 'gene_keys';
        if (raw.includes('final verdict') || raw.includes('verdict'))
            return 'verdict';
        return 'western';
    };
    ...
    try {
        const response = await llm_1.llm.generate(prompt, 'lyrics-generation', {
...
Return ONLY this JSON (no other text):
{
  ...
  "minimaxPrompt": "${system === 'vedic' ? 'Contemplative Indian classical fusion. Sitar, bansuri flute, tabla. Slow, introspective.' : 'Quiet piano ballad in Leonard Cohen style. Acoustic, contemplative.'}"
            ...
            const retryResponse = await llm_1.llm.generate(simplePrompt, 'lyrics-generation-retry', {
                maxTokens: 1500,
                temperature: 0.7,
            });
...
//# sourceMappingURL=lyricsGeneration.js.map