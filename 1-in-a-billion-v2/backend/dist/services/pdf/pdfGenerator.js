...
exports.generateReadingPDF = generateReadingPDF;
exports.generateChapterPDF = generateChapterPDF;
...
const GARAMOND = path_1.default.join(FONTS_DIR, 'EBGaramond-Regular.ttf');
const GARAMOND_BOLD = path_1.default.join(FONTS_DIR, 'EBGaramond-Bold.ttf');
const PLAYFAIR_BOLD = path_1.default.join(FONTS_DIR, 'PlayfairDisplay_700Bold.ttf');
// CJK & Devanagari fonts (Noto Sans family)
const NOTO_SANS_JP = path_1.default.join(FONTS_DIR, 'NotoSansJP-Regular.ttf');
const NOTO_SANS_SC = path_1.default.join(FONTS_DIR, 'NotoSansSC-Regular.ttf');
const NOTO_SANS_KR = path_1.default.join(FONTS_DIR, 'NotoSansKR-Regular.ttf');
const NOTO_SANS_DEVANAGARI = path_1.default.join(FONTS_DIR, 'NotoSansDevanagari-Regular.ttf');
/** For CJK/Hindi, Latin fonts have zero glyphs → swap to Noto Sans */
function getNonLatinFont(lang) {
    switch (lang) {
...
function translateLabel(label, lang) {
    if (!lang || lang === 'en')
        return label;
    return PDF_TRANSLATIONS[label]?.[lang] || label;
}
function buildOverlayCoverTitle(title) {
...
function formatPersonCoverLine(person) {
 ...
 */
async function prepareImageForPdf(raw) {
...
function isLikelySubheadline(paragraph) {
    ...
    if (upperWords / words.length >= 0.9)
        return true;
    if (/^(?:THE|The|A|An|What|When|How|Why|Who)\s+/i.test(text) && words.length >= 4)
        return true;
    const titleLikeWords = words.filter((w) => /^[A-Z][A-Za-z'’\-]*$/.test(w) || /^[A-Z0-9'’\-]+$/.test(w)).length;
    if (titleLikeWords / words.length >= 0.85)
        return true;
    return false;
}
function splitEmbeddedHeadline(paragraph) {
    ...
    if (isLikelySubheadline(stripped))
        return { headline: stripped, body: '' };
    return { body: stripped };
}
function isOrphanLeadLine(paragraph) {
    const text = String(paragraph || '').trim();
    if (!text)
        return false;
    if (isLikelySubheadline(text))
        return false;
    if (/^[#\-—–]/.test(text))
        return false;
    if (/[.!?;:]$/.test(text))
...
function isInlineSentenceHeadlineCandidate(sentence) {
    const text = String(sentence || '').trim();
    if (!text)
        return false;
    if (!/^(The|A|An|What|When|How|Why|Who)\s+/i.test(text))
        return false;
    if (!/[.!?]$/.test(text))
        return false;
    if (/[,:;]/.test(text))
    ...
    if (!headlineNouns.some((n) => lower.includes(n)))
        return false;
    return true;
}
function isInlineTitleHeadlineCandidate(candidate) {
    const text = String(candidate || '').trim();
    if (!text)
        return false;
    if (!/^(?:The|A|An|What|When|How|Why|Who)\s+/.test(text))
        return false;
    if (/[,:;.!?]/.test(text))
        return false;
    const words = text.split(/\s+/).filter(Boolean);
    ...
    if (meaningfulWords.length > 0 && titleLikeWords / meaningfulWords.length < 0.8)
        return false;
...
function renderReadingText(doc, text, hasPlayfairBold, allowInferredHeadlines = true) {
    ...
    const inlineTitleHeadingRe = new RegExp(`([.!?])\\s+((?:The|A|An|What|When|How|Why|Who)\\s+${titleToken}(?:\\s+${titleToken}){2,14})\\s+([A-Z][a-z])`, 'g');
    const normalized = String(text || '')
        ...
        if (!isInlineTitleHeadlineCandidate(candidate))
            return m;
        return `${p1}\n\n${candidate}\n\n${p3}`;
    })
        ...
        if (!isInlineSentenceHeadlineCandidate(candidate))
            return m;
        return `${p1}\n\n${candidate}\n\n${p3}`;
    });
    ...
    for (const line of rawParagraphs) {
        ...
        const prevLooksHeadline = Boolean(prev) &&
            prevWords >= 4 &&
            prevWords <= 18 &&
            !/[.!?]$/.test(prev) &&
            /^(?:THE|The|A|An)\s+/.test(prev);
        const lineLooksShortTail = /^[A-Z][A-Za-z'’\-]*(?:\s+[A-Z][A-Za-z'’\-]*){0,2}$/.test(line) &&
            !/[,:;.!?]$/.test(line);
    ...
    const wordsIn = (value) => value.split(/\s+/).filter(Boolean).length;
...
function extractChartSignatureFooter(rawText) {
 ...
 */
function stripCompatibilitySnapshot(text) {
    const s = String(text || '');
    ...
    if (dashMarker >= 0)
        return s.slice(0, dashMarker).trim();
    // Match standalone "COMPATIBILITY SNAPSHOT:" line
    const directMarker = s.search(/\nCOMPATIBILITY SNAPSHOT\s*:/i);
    if (directMarker >= 0)
        return s.slice(0, directMarker).trim();
    return s;
}
function buildSystemGlossary(system) {
...
function pickGlossaryEntries(system, signatureText) {
...
function renderSignatureGlossaryPage(doc, blocks, hasPlayfairBold) {
    if (!blocks.length)
        return;
    doc.addPage();
...
function renderChartBox(doc, params) {
...
function renderChartReferenceText(doc, chartReferencePage, hasPlayfairBold, chartReferencePageRight) {
 ...
 */
function extractFinalIronyVerdict(reading) {
    ...
    if (!Number.isFinite(score) || !verdictText)
        return null;
    return {
 ...
 */
function extractCompatibilityRows(reading, _appendix) {
    const text = String(reading || '');
    ...
    while ((m = scoreBlockRe.exec(normalizedText)) !== null) {
        ...
        if (!rawLabel || !Number.isFinite(score100))
            continue;
        // Skip header/meta lines and common false positives from reading prose
        if (/^(COMPATIBILITY|SCORING|FORMAT|OUTPUT|STRUCTURE|STYLE|CHAPTER|SECTION|PART|BORN|AGE|HOUSE|PAGE|VERSE|LINE|STEP|YEAR|DAY|MONTH)/i.test(rawLabel))
            continue;
        // Labels should be at least 3 chars and no more than 60 (avoids matching random prose lines)
        if (rawLabel.length < 3 || rawLabel.length > 60)
            continue;
...
function renderCompatibilitySnapshotPage(doc, rows, hasPlayfairBold, finalVerdict, lang) {
    if (!rows.length && !finalVerdict)
        return;
    doc.addPage();
...
function renderLegacyChartReferenceText(doc, chartReferencePage, hasPlayfairBold) {
...
async function generateReadingPDF(options) {
    const outputDir = path_1.default.resolve(__dirname, '../../../generated-pdfs');
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (notoFont && fs_1.default.existsSync(notoFont)) {
                ...
                console.log(`[pdfGenerator] Using Noto font for ${options.outputLanguage}: ${path_1.default.basename(notoFont)}`);
            }
            else {
                if (fs_1.default.existsSync(GARAMOND)) {
                    doc.registerFont('Garamond', GARAMOND);
                }
                if (fs_1.default.existsSync(GARAMOND_BOLD)) {
            ...
            const hasPlayfairBold = notoFont ? true : fs_1.default.existsSync(PLAYFAIR_BOLD);
            if (!notoFont && fs_1.default.existsSync(PLAYFAIR_BOLD)) {
            ...
            doc.moveDown(2);
            const renderPersonCover = (person, showName = true) => {
                if (showName) {
                ...
                const birthDate = formatBirthDate(person.birthDate);
                if (birthDate)
                    birthBits.push(birthDate);
                if (person.birthTime)
                    birthBits.push(person.birthTime);
                if (person.birthPlace)
                    birthBits.push(person.birthPlace);
                if (person.timezone && !person.birthPlace)
                    birthBits.push(person.timezone);
            ...
            const renderCoverPersonLabel = (label) => {
            ...
            const cleanedChapters = options.chapters.map((chapter) => {
                ...
                if (options.person2)
                    capture('person2Reading', `${options.person2.name} Signature`);
                if (options.person2)
                    capture('overlayReading', `${options.person1.name} & ${options.person2.name} Signature`);
                ...
                if (cleaned.overlayReading)
                    cleaned.overlayReading = stripCompatibilitySnapshot(cleaned.overlayReading);
                if (cleaned.verdict)
                    cleaned.verdict = stripCompatibilitySnapshot(cleaned.verdict);
                return cleaned;
            });
            // Body text
            ...
            if (options.type === 'overlay') {
                ...
                if (options.compatibilityScores && options.compatibilityScores.length > 0) {
                    const rows = options.compatibilityScores.map((s) => ({
                        label: s.label,
                        score: s.scoreTen,
...
async function generateChapterPDF(chapterNumber, chapter, person1, person2, coupleImageUrl, extras) {
    ...
    if (contentFields.length > 1) {
        throw new Error(`CRITICAL PDF ERROR: Multiple reading fields have content (${contentFields.length} fields). ` +
