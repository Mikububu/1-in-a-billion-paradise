...
exports.extractVedicEssences = extractVedicEssences;
exports.extractHumanDesignEssences = extractHumanDesignEssences;
exports.extractGeneKeysEssences = extractGeneKeysEssences;
exports.extractKabbalahlahEssences = extractKabbalahlahEssences;
exports.extractAllEssences = extractAllEssences;
exports.generateEssencesFromPlacements = generateEssencesFromPlacements;
/**
 * Extract Vedic essences from reading text
 * Looks for: Nakshatra, Pada, Lagna, Moon sign
 */
function extractVedicEssences(readingText) {
    if (!readingText)
        return null;
    const result = {};
    // Extract Nakshatra - patterns like "Magha Nakshatra", "in Ashwini", "Moon dwells in Bharani"
    const nakshatraMatch = readingText.match(/(?:in |dwells in |Moon.*?in |Nakshatra[:\s]+)([A-Z][a-z]+)\s*(?:Nakshatra)?/i);
    ...
    if (padaMatch) {
        ...
        const padaMap = { '1': 1, '2': 2, '3': 3, '4': 4, 'one': 1, 'two': 2, 'three': 3, 'four': 4 };
 ...
 */
function extractHumanDesignEssences(readingText) {
    if (!readingText)
        return null;
    const result = {};
    // Extract Type - patterns like "You are a Generator", "Manifesting Generator", "Projector", etc.
    const typeMatch = readingText.match(/(?:You are a |Your type is |as a )(Manifesting Generator|Generator|Projector|Manifestor|Reflector)/i) || readingText.match(/(Manifesting Generator|Generator|Projector|Manifestor|Reflector)\s+type/i);
    if (typeMatch) {
 ...
 */
function extractGeneKeysEssences(readingText) {
    if (!readingText)
        return null;
    const result = {};
 ...
 */
function extractKabbalahlahEssences(readingText) {
    if (!readingText)
        return null;
    const result = {};
 ...
 */
function generateEssencesFromPlacements(placements) {
    const essences = {};
    // Western
...
//# sourceMappingURL=essenceExtractionService.js.map