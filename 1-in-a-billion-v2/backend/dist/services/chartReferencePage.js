...
exports.buildChartReferencePage = buildChartReferencePage;
// ─── shared helpers ──────────────────────────────────────────────────────────
...
function computeAgeFromBirthDate(raw, now) {
    const value = String(raw || '').trim();
    const m = value.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!m)
        return undefined;
...
function extractRawLines(chartData) {
    return String(chartData || '').split('\n').map((l) => normalizeSpacing(l)).filter(Boolean);
}
function extractEvidenceLines(chartData) {
...
function pickAll(lines, re) {
    return lines.filter((l) => re.test(l));
}
function divider() {
    return '---------------------------------------';
}
function header(personName, birth, system, generatedAt) {
    ...
    if (bornLine)
        lines.push(`Born: ${bornLine}`);
    if (typeof computedAge === 'number')
        lines.push(`Age: ${computedAge}`);
    lines.push(divider(), '');
    return lines;
}
// ─── WESTERN ─────────────────────────────────────────────────────────────────
function buildWesternReferencePage(input) {
    ...
    const out = [...header(input.personName, input.birth, 'Western Astrology', input.generatedAt)];
    out.push('BIG THREE');
    if (sun)
        out.push(`  Sun:      ${sun.replace(/^SUN:\s*/i, '')}`);
    if (moon)
        out.push(`  Moon:     ${moon.replace(/^MOON:\s*/i, '')}`);
    if (asc)
        out.push(`  Rising:   ${asc.replace(/^(Ascendant|Rising):\s*/i, '')}`);
    out.push('');
    out.push('PERSONAL PLANETS');
    if (mercury)
        out.push(`  Mercury:  ${mercury.replace(/^MERCURY:\s*/i, '')}`);
    if (venus)
        out.push(`  Venus:    ${venus.replace(/^VENUS:\s*/i, '')}`);
    if (mars)
        out.push(`  Mars:     ${mars.replace(/^MARS:\s*/i, '')}`);
    out.push('');
    out.push('OUTER PLANETS');
    if (jupiter)
        out.push(`  Jupiter:  ${jupiter.replace(/^JUPITER:\s*/i, '')}`);
    if (saturn)
        out.push(`  Saturn:   ${saturn.replace(/^SATURN:\s*/i, '')}`);
    if (uranus)
        out.push(`  Uranus:   ${uranus.replace(/^URANUS:\s*/i, '')}`);
    if (neptune)
        out.push(`  Neptune:  ${neptune.replace(/^NEPTUNE:\s*/i, '')}`);
    if (pluto)
        out.push(`  Pluto:    ${pluto.replace(/^PLUTO:\s*/i, '')}`);
    out.push('');
    out.push('NODES');
    if (northNode)
        out.push(`  North Node: ${northNode.replace(/^North Node:\s*/i, '')}`);
    if (southNode)
        out.push(`  South Node: ${southNode.replace(/^South Node:\s*/i, '')}`);
    out.push('');
    ...
    out.push('PROFECTION');
    if (profectedSign)
        out.push(`  ${normalizeSpacing(profectedSign)}`);
    if (lordOfYear)
        out.push(`  ${normalizeSpacing(lordOfYear)}`);
    out.push(divider());
    return out.join('\n').trim();
}
// ─── VEDIC ───────────────────────────────────────────────────────────────────
function buildVedicReferencePage(input) {
    ...
    const out = [...header(input.personName, input.birth, 'Vedic Astrology (Jyotish)', input.generatedAt)];
    out.push('LAGNA (ASCENDANT)');
    if (lagna)
        out.push(`  Sign:       ${lagna}`);
    if (lagnaLord)
        out.push(`  Lord:       ${lagnaLord}`);
    out.push('');
    out.push('GRAHAS (SIDEREAL)');
    const graha = (label, val) => val ? out.push(`  ${label.padEnd(10)} ${val}`) : undefined;
    graha('Surya:', surya);
    ...
    graha('Ketu:', ketu);
    if (janmaNakshatra)
        out.push(`  Janma Nak:  ${janmaNakshatra}`);
    out.push('');
    out.push('VIMSHOTTARI DASHA');
    if (mahadasha)
        out.push(`  Mahadasha:  ${mahadasha}`);
    if (antardasha)
        out.push(`  Antardasha: ${antardasha}`);
    out.push('');
    if (seventh) {
    ...
    if (loudestLines.length > 0) {
        ...
        for (const l of loudestLines.slice(0, 3))
            out.push(`  ${l}`);
        out.push('');
    }
    if (temperature) {
...
function buildHumanDesignReferencePage(input) {
    ...
    const out = [...header(input.personName, input.birth, 'Human Design', input.generatedAt)];
    out.push('DESIGN');
    if (type)
        out.push(`  Type:       ${type}`);
    if (strategy)
...
function buildGeneKeysReferencePage(input) {
    ...
    const out = [...header(input.personName, input.birth, 'Gene Keys', input.generatedAt)];
    out.push('ACTIVATION SEQUENCE (Prime Gifts)');
    for (const { label, val } of spheres.slice(0, 4)) {
        if (val)
            out.push(`  ${label.padEnd(12)} ${val}`);
    }
    out.push('');
...
function buildKabbalahReferencePage(input) {
    ...
    const out = [...header(input.personName, input.birth, 'Kabbalah (Hermetic)', input.generatedAt)];
    out.push('FOUR WORLDS');
    if (dominantWorld)
        out.push(`  Dominant: ${dominantWorld}`);
    if (voidWorlds)
        out.push(`  Void:     ${voidWorlds}`);
    out.push('');
    out.push('SEFIROT');
    if (strongSefirot.length > 0) {
        out.push('  Strong:');
        for (const s of strongSefirot)
            out.push(`    ${s}`);
    }
    if (moderateSefirot.length > 0) {
        out.push('  Moderate:');
        for (const s of moderateSefirot)
            out.push(`    ${s}`);
    }
    out.push('');
    out.push('LETTER SIGNATURE');
    if (sunLetter)
        out.push(`  Sun:    ${sunLetter}`);
    if (moonLetter)
        out.push(`  Moon:   ${moonLetter}`);
    if (risingLetter)
        out.push(`  Rising: ${risingLetter}`);
    out.push('');
    out.push('SHADOW WORK');
    if (shadowAxis)
        out.push(`  Primary Axis: ${shadowAxis}`);
    if (klipoth)
        out.push(`  Active Klipoth: ${klipoth}`);
    out.push('');
    out.push(divider());
    return out.join('\n').trim();
}
// ─── PUBLIC ENTRY POINT ──────────────────────────────────────────────────────
function buildChartReferencePage(input) {
...
//# sourceMappingURL=chartReferencePage.js.map