...
exports.stripKabbalahChartData = stripKabbalahChartData;
exports.buildKabbalahTriggerPrompt = buildKabbalahTriggerPrompt;
exports.buildKabbalahWritingPrompt = buildKabbalahWritingPrompt;
const triggerConfig_1 = require("./triggerConfig");
const kabbalah_1 = require("../../prompts/systems/kabbalah");
/**
 ...
 */
function stripKabbalahChartData(raw) {
    ...
    for (const line of lines) {
        ...
        if (/^SEFIROTIC STRUCTURE/.test(t)) {
            inTikkun = false;
            inSefirot = true;
            out.push(line);
            continue;
        }
        if (/^DOMINANT \(STRONG\)/.test(t)) {
            inDominantStrong = true;
            inModerate = false;
            inWeak = false;
            strongCount = 0;
            out.push(line);
        ...
        if (/^MODERATE:/.test(t)) {
            inDominantStrong = false;
            inModerate = true;
            continue;
        }
        if (/^WEAK\/UNDERDEVELOPED/.test(t)) {
            inModerate = false;
            inWeak = true;
            continue;
        }
        if (/^VOID\/DEFICIENT SEFIROT/.test(t)) {
            inWeak = false;
            out.push(line);
            continue;
        }
        if (/^- Pillar Balance/.test(t)) {
            continue;
        }
        if (/^FOUR WORLDS/.test(t)) {
            inSefirot = false;
            inFourWorlds = true;
            out.push(line);
            continue;
        }
        if (/^LETTER SIGNATURE/.test(t)) {
            inFourWorlds = false;
            inLetterSig = true;
            continue;
        }
        ...
        if (/^CURRENT SPIRITUAL WEATHER/.test(t)) {
            inKlipoth = false;
            inTransitWeather = true;
            continue;
        }
        if (/^MODALITY BALANCE/.test(t)) {
            inTransitWeather = false;
            inModality = true;
            continue;
        }
        if (/^POLICY:/.test(t)) {
        ...
        // Header - keep
        if (/KABBALAH PROFILE/.test(t)) {
            out.push(line);
            continue;
        }
        // Hebrew date - keep
        if (/^HEBREW BIRTH DATE/.test(t)) {
            out.push(line);
            continue;
        }
        if (/^- [0-9]/.test(t) && out.some(l => l.includes('HEBREW'))) {
            out.push(line);
            continue;
        }
        if (inTikkun) {
            if (/^- (Hebrew Month|Tikkun Name|Correction|Trap|Gift):/.test(t))
                out.push(line);
            continue;
        }
        if (inSefirot) {
            continue;
        } // Section header only kept above
        if (inDominantStrong) {
        ...
        if (inModerate || inWeak) {
            continue;
        }
        if (inFourWorlds) {
            // Keep dominant and void lines only
            if (/^- Dominant World:/.test(t) || /^- Void Worlds:/.test(t)) {
                out.push(line);
            }
            continue;
        }
        ...
        if (inKlipoth) {
            ...
            if (/^- Primary Shadow Axis:/.test(t)) {
                out.push(line);
            }
            continue;
        }
        if (inTransitWeather || inModality) {
            continue;
        }
    }
...
// ─── 2. TRIGGER PROMPT ───────────────────────────────────────────────────────
function buildKabbalahTriggerPrompt(params) {
...
// ─── 3. WRITING PROMPT ───────────────────────────────────────────────────────
function buildKabbalahWritingPrompt(params) {
    ...
    const kabbalahGuidance = (0, kabbalah_1.buildKabbalahSection)(false);
    return [
...
//# sourceMappingURL=kabbalahTrigger.js.map