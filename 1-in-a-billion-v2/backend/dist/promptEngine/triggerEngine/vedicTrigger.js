...
exports.stripVedicChartData = stripVedicChartData;
exports.buildVedicTriggerPrompt = buildVedicTriggerPrompt;
exports.buildVedicWritingPrompt = buildVedicWritingPrompt;
const triggerConfig_1 = require("./triggerConfig");
const vedic_1 = require("../../prompts/systems/vedic");
/**
 ...
 */
function stripVedicChartData(raw) {
    ...
    for (const line of lines) {
        const t = line.trim();
        // Section routing
        if (/^GRAHA POSITIONS:/.test(t)) {
            inGrahas = true;
            inBhava = false;
            out.push(line);
            continue;
        }
        if (/^BHAVA OCCUPANCY/.test(t)) {
            inGrahas = false;
            inBhava = true;
            continue;
        }
        if (/^PRESSURIZED BHAVAS/.test(t)) {
            inBhava = false;
            inPressurized = true;
            continue;
        }
        if (/^EMPTY ANGULAR BHAVAS/.test(t)) {
            inPressurized = false;
            inEmpty = true;
            continue;
        }
        ...
        if (/^VIMSHOTTARI DASHA/.test(t)) {
            in7th = false;
            inDasha = true;
            out.push(line);
            continue;
        }
        if (/^FULL DASHA SEQUENCE/.test(t)) {
            inFullSequence = true;
            continue;
        }
        if (/^NAVAMSHA/.test(t)) {
            inDasha = false;
            inFullSequence = false;
            inNavamsha = true;
            out.push(line);
            continue;
        }
        // Header lines - keep
        if (/^[A-Z\s]+VEDIC CHART/.test(t)) {
            out.push(line);
            continue;
        }
        if (/^- (Birth|Place|Current Age|Ayanamsa):/.test(t)) {
            out.push(line);
            continue;
        }
        ...
        if (/^- Chandra Rashi:/.test(t) || /^- Janma Nakshatra:/.test(t)) {
            out.push(line);
            continue;
        }
        ...
        if (inBhava || inPressurized || inEmpty || in7th) {
            continue;
        }
        // Dasha - keep current Maha + Antardasha only
        if (inDasha && !inFullSequence) {
            if (/^- Current (Mahadasha|Antardasha):/.test(t)) {
                out.push(line);
            }
            continue;
        }
        ...
        if (inNavamsha) {
            if (/^- Navamsha Lagna:/.test(t)) {
                out.push(line);
                continue;
            }
            if (/^- (Surya|Chandra|Mangal|Shani|Rahu|Ketu)/.test(t)) {
                out.push(line);
                continue;
            }
            if (/^NAVAMSHA/.test(t)) {
...
// ─── 2. TRIGGER PROMPT ───────────────────────────────────────────────────────
function buildVedicTriggerPrompt(params) {
...
// ─── 3. WRITING PROMPT ───────────────────────────────────────────────────────
function buildVedicWritingPrompt(params) {
    ...
    const vedicGuidance = (0, vedic_1.buildVedicSection)(false, 7);
    return [
...
//# sourceMappingURL=vedicTrigger.js.map