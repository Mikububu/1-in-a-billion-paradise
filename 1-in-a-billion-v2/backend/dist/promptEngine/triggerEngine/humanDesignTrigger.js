...
exports.stripHDChartData = stripHDChartData;
exports.buildHDTriggerPrompt = buildHDTriggerPrompt;
exports.buildHDWritingPrompt = buildHDWritingPrompt;
const triggerConfig_1 = require("./triggerConfig");
const human_design_1 = require("../../prompts/systems/human-design");
/**
...
function stripHDChartData(raw) {
    ...
    for (const line of lines) {
        ...
        if (/^PERSONALITY ACTIVATIONS/.test(t)) {
            inPersonality = true;
            inDesign = false;
            inGates = false;
            out.push(line);
...
function extractGateNumbers(raw) {
    const gateNumbers = [];
    let inGates = false;
    for (const line of raw.split('\n')) {
        ...
        if (inGates) {
            ...
            if (match)
                gateNumbers.push(parseInt(match[1]));
        }
    }
    return gateNumbers.length > 0 ? gateNumbers.join(', ') : 'none found';
}
// ─── 2. TRIGGER PROMPT ───────────────────────────────────────────────────────
function buildHDTriggerPrompt(params) {
...
// ─── 3. WRITING PROMPT ───────────────────────────────────────────────────────
function buildHDWritingPrompt(params) {
    ...
    const hdGuidance = (0, human_design_1.buildHumanDesignSection)(false);
    return [
...
//# sourceMappingURL=humanDesignTrigger.js.map