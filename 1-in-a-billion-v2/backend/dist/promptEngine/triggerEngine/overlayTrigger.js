...
exports.stripWesternOverlayData = stripWesternOverlayData;
exports.stripVedicOverlayData = stripVedicOverlayData;
exports.stripHDOverlayData = stripHDOverlayData;
exports.stripGeneKeysOverlayData = stripGeneKeysOverlayData;
exports.stripKabbalahOverlayData = stripKabbalahOverlayData;
exports.buildWesternOverlayTriggerPrompt = buildWesternOverlayTriggerPrompt;
exports.buildVedicOverlayTriggerPrompt = buildVedicOverlayTriggerPrompt;
exports.buildHDOverlayTriggerPrompt = buildHDOverlayTriggerPrompt;
exports.buildGeneKeysOverlayTriggerPrompt = buildGeneKeysOverlayTriggerPrompt;
exports.buildKabbalahOverlayTriggerPrompt = buildKabbalahOverlayTriggerPrompt;
exports.buildWesternOverlayWritingPrompt = buildWesternOverlayWritingPrompt;
exports.buildVedicOverlayWritingPrompt = buildVedicOverlayWritingPrompt;
exports.buildHDOverlayWritingPrompt = buildHDOverlayWritingPrompt;
exports.buildGeneKeysOverlayWritingPrompt = buildGeneKeysOverlayWritingPrompt;
exports.buildKabbalahOverlayWritingPrompt = buildKabbalahOverlayWritingPrompt;
const triggerConfig_1 = require("./triggerConfig");
const westernTrigger_1 = require("./westernTrigger");
const vedicTrigger_1 = require("./vedicTrigger");
const humanDesignTrigger_1 = require("./humanDesignTrigger");
const geneKeysTrigger_1 = require("./geneKeysTrigger");
const kabbalahTrigger_1 = require("./kabbalahTrigger");
function combineLabeled(person1Stripped, person2Stripped) {
    return [
        'PERSON1 CHART:',
        person1Stripped,
        '',
...
function buildOverlayTriggerPromptBase(params) {
    ...
    const relationalTrigger = triggerConfig_1.RELATIONAL_TRIGGER_LABEL;
    return [
...
function buildOverlayWritingPromptBase(params) {
    ...
    const relationalTrigger = triggerConfig_1.RELATIONAL_TRIGGER_LABEL;
    const relationalTriggerTitle = triggerConfig_1.RELATIONAL_TRIGGER_TITLE;
    return [
        narratorIdentity,
        triggerConfig_1.CORE_FAIRYTALE_SEED_OVERLAY,
 ...
 */
function extractWesternOverlayExtras(raw) {
    const extras = [];
 ...
 */
function extractVedicOverlayExtras(raw) {
    const extras = [];
 ...
 */
function extractHDOverlayExtras(raw) {
    for (const line of raw.split('\n')) {
 ...
 */
function extractKabbalahOverlayExtras(raw) {
    const extras = [];
    let inKlipoth = false;
    for (const line of raw.split('\n')) {
        const t = line.trim();
        if (/^KLIPOTHIC RISK/.test(t)) {
            inKlipoth = true;
            continue;
        }
        if (inKlipoth) {
            if (/^- (Active Klipoth|Hard Aspects):/.test(t)) {
                extras.push(line);
            }
            else if (/^[A-Z]/.test(t) && !/^- /.test(t)) {
                inKlipoth = false;
            }
        }
    }
...
function stripWesternOverlayData(person1Raw, person2Raw) {
    const p1Extra = extractWesternOverlayExtras(person1Raw);
    const p2Extra = extractWesternOverlayExtras(person2Raw);
    return [
        'PERSON1 CHART:',
        (0, westernTrigger_1.stripWesternChartData)(person1Raw) + p1Extra,
        '',
        'PERSON2 CHART:',
        (0, westernTrigger_1.stripWesternChartData)(person2Raw) + p2Extra,
    ].join('\n');
}
function stripVedicOverlayData(person1Raw, person2Raw) {
    const p1Extra = extractVedicOverlayExtras(person1Raw);
    const p2Extra = extractVedicOverlayExtras(person2Raw);
    return [
        'PERSON1 CHART:',
        (0, vedicTrigger_1.stripVedicChartData)(person1Raw) + p1Extra,
        '',
        'PERSON2 CHART:',
        (0, vedicTrigger_1.stripVedicChartData)(person2Raw) + p2Extra,
    ].join('\n');
}
function stripHDOverlayData(person1Raw, person2Raw) {
    const p1Extra = extractHDOverlayExtras(person1Raw);
    const p2Extra = extractHDOverlayExtras(person2Raw);
    return [
        'PERSON1 CHART:',
        (0, humanDesignTrigger_1.stripHDChartData)(person1Raw) + p1Extra,
        '',
        'PERSON2 CHART:',
        (0, humanDesignTrigger_1.stripHDChartData)(person2Raw) + p2Extra,
    ].join('\n');
}
function stripGeneKeysOverlayData(person1Raw, person2Raw) {
    // Gene Keys already keeps everything - no extras needed
    return combineLabeled((0, geneKeysTrigger_1.stripGeneKeysChartData)(person1Raw), (0, geneKeysTrigger_1.stripGeneKeysChartData)(person2Raw));
}
function stripKabbalahOverlayData(person1Raw, person2Raw) {
    const p1Extra = extractKabbalahOverlayExtras(person1Raw);
    const p2Extra = extractKabbalahOverlayExtras(person2Raw);
    return [
        'PERSON1 CHART:',
        (0, kabbalahTrigger_1.stripKabbalahChartData)(person1Raw) + p1Extra,
        '',
        'PERSON2 CHART:',
        (0, kabbalahTrigger_1.stripKabbalahChartData)(person2Raw) + p2Extra,
    ].join('\n');
}
function buildWesternOverlayTriggerPrompt(params) {
    return buildOverlayTriggerPromptBase({
        ...params,
        systemInstruction: `Use synastry cross-aspects (especially Venus, Mars, Jupiter contacts), angular overlays, and Saturn/Pluto contact pressure to identify the precise ${triggerConfig_1.RELATIONAL_TRIGGER_LABEL}.`,
    });
}
function buildVedicOverlayTriggerPrompt(params) {
    return buildOverlayTriggerPromptBase({
        ...params,
...
function buildHDOverlayTriggerPrompt(params) {
    return buildOverlayTriggerPromptBase({
        ...params,
        systemInstruction: 'Use defined/undefined center interplay, channel completion pressure (check active gates for electromagnetic connections), and authority mismatch to identify the core relational dynamic.',
    });
}
function buildGeneKeysOverlayTriggerPrompt(params) {
    return buildOverlayTriggerPromptBase({
        ...params,
        systemInstruction: `Use specific Gene Key numbers and their Shadow frequency names. Identify shadow-frequency resonance, gift-shadow mismatch, and Programming Partner dynamics between profiles to find the exact ${triggerConfig_1.RELATIONAL_TRIGGER_LABEL} loop. Name the Keys involved (e.g., "Gene Key 44 Shadow of Interference triggering Gene Key 22 Shadow of Dishonour").`,
    });
}
function buildKabbalahOverlayTriggerPrompt(params) {
    return buildOverlayTriggerPromptBase({
        ...params,
        systemInstruction: `Use Tikkun alignment/conflict, sephirotic imbalance, and klipothic interference pressure (check active klipoth shells and hard aspects for specific shadow interactions) to identify the ${triggerConfig_1.RELATIONAL_TRIGGER_LABEL}.`,
    });
}
function buildWesternOverlayWritingPrompt(params) {
    return buildOverlayWritingPromptBase({
        ...params,
        narratorIdentity: 'You are a novelist who has seen charts like these before, and you know what this collision would look like if it happened - because you have watched similar mathematics play out in different bodies across different centuries.',
        overlayGuidance: [
...
function buildHDOverlayWritingPrompt(params) {
    return buildOverlayWritingPromptBase({
        ...params,
        narratorIdentity: 'You are a novelist who understands the body as a receiver and relationship as a circuit - exploring what would regulate and what would overload if these two designs shared a space.',
        overlayGuidance: [
...
function buildGeneKeysOverlayWritingPrompt(params) {
    return buildOverlayWritingPromptBase({
        ...params,
        narratorIdentity: 'You are a Gene Keys reader and novelist studying what two people could activate in each other that neither could activate alone. Name specific Gene Key numbers, Shadow frequencies, and Gift potentials. Use Gene Keys terminology: Shadow, Gift, Siddhi, frequency, contemplation, Activation Sequence, Venus Sequence. Explain terms naturally on first use. Ground every insight in specific Keys from the chart data.',
        overlayGuidance: [
...
function buildKabbalahOverlayWritingPrompt(params) {
    return buildOverlayWritingPromptBase({
        ...params,
        narratorIdentity: 'You are a novelist who understands correction as collision, exploring what would happen if two soul corrections either accelerated or obstructed each other.',
        overlayGuidance: [
