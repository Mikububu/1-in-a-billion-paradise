...
exports.compactWesternChartDataForDigest = compactWesternChartDataForDigest;
exports.buildWesternChartDigestPrompt = buildWesternChartDigestPrompt;
exports.extractEvidenceLinesFromDigest = extractEvidenceLinesFromDigest;
exports.validateWesternDigestAgainstChartData = validateWesternDigestAgainstChartData;
exports.extractWoundFromDigest = extractWoundFromDigest;
const DIGEST_HEADER = 'WESTERN_CHART_DIGEST_V3';
function compactWesternChartDataForDigest(chartData) {
...
function buildWesternChartDigestPrompt(params) {
    ...
    return [
        ...
        '',
        'NARRATIVE_ARC:',
        '- THE_WOUND: <One sentence. The most vulnerable, most hidden thing in this chart. Not the behavior - the thing underneath the behavior. Not what they do, but what they cannot let themselves know. Must be specific enough that no other human chart would produce the same sentence. Must cost something to read.>',
        '- THE_DEFENSE: <One sentence. What this person does, has always done, to keep THE_WOUND from being seen - by others, by themselves. The structure they built around it.>',
        '- THE_COST: <One sentence. What the defense destroys over time. The specific form of loneliness or failure that results from protecting the wound so well.>',
        '- ACT_1_SURFACE: <2-3 sentences. The person as they appear. What draws people in. What they lead with. This is the first 1500 words of the reading - compelling, specific, slightly unsettling.>',
        '- ACT_2_BENEATH: <2-3 sentences. Where the reading breaks through the surface and finds the wound. The scene where the defense becomes visible as a defense. The moment the reader stops feeling flattered and starts feeling recognized.>',
        '- ACT_3_RECKONING: <2-3 sentences. What the current transits are forcing. What is being demolished. What cannot continue. NOT resolution or growth - the specific pressure right now. What this person is standing in front of that they cannot avoid.>',
        '- LANDING_TEMPERATURE: <One sentence. The emotional state the reading should leave the reader in. Not hopeful. Not destroyed. Specific. Example: "The specific vertigo of someone who just realized the map they have been using is a map of someone else\'s country." This is the final note the reading ends on.>',
        '- WHAT_THIS_READING_MUST_NOT_DO: <One sentence. The specific flattery trap this chart invites. The thing the model will want to say that makes the subject feel sophisticated or special rather than seen. Name it explicitly so the writer avoids it.>',
        '',
...
function extractEvidenceLinesFromDigest(digest) {
    ...
    for (let i = start + 1; i < lines.length; i += 1) {
        ...
        if (!trimmed)
            continue;
        if (trimmed === 'SEEDS:' || trimmed.endsWith(':') && /^[A-Z0-9_]+:$/.test(trimmed))
            break;
...
function validateWesternDigestAgainstChartData(params) {
    ...
    const requiredSections = ['NARRATIVE_ARC', 'THE_WOUND', 'LANDING_TEMPERATURE'];
    for (const section of requiredSections) {
        if (!digest.includes(section)) {
            return { ok: false, evidenceLines, reason: `Missing required section: ${section}` };
        }
    }
    return { ok: true, evidenceLines };
}
function extractWoundFromDigest(digest) {
    const text = String(digest || '');
    const match = text.match(/^- THE_WOUND:\s*(.+)$/m);
    return match ? match[1].trim() : null;
}
//# sourceMappingURL=westernDigest.js.map