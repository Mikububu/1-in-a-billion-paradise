...
const layerRegistry_1 = require("./layerRegistry");
...
const INCARNATION_SYSTEM_PROMPTS = {
    hellenistic: [
...
function capLayer(name, content, maxChars, stats) {
...
function dropSecondPersonLines(content) {
...
function resolveStyleLayerId(directive) {
    const requested = directive?.sharedWritingStyleLayerId;
    if (requested && layerRegistry_1.STYLE_LAYER_REGISTRY.files[requested]) {
        return requested;
    }
    return layerRegistry_1.STYLE_LAYER_REGISTRY.defaultLayerId;
}
function extractSystemPromptFromStyleLayer(styleLayerId, primarySystem, readingKind) {
...
function stripIncarnationIdentitySection(style) {
...
function resolveVerdictLayerId(directive) {
    const requested = directive?.finalVerdictLayerId;
    if (requested && layerRegistry_1.VERDICT_LAYER_REGISTRY.files[requested]) {
        return requested;
    }
    return layerRegistry_1.VERDICT_LAYER_REGISTRY.defaultLayerId;
}
function layerModeFromReadingKind(readingKind) {
    return readingKind === 'individual' ? 'individual' : 'synastry';
}
function resolveSystemLayerId(system, readingKind, directive) {
    const registry = layerRegistry_1.SYSTEM_LAYER_REGISTRY[system];
...
function modeRules(readingKind) {
    if (readingKind === 'synastry') {
        return [
            ...
            '[2 sentences: what kind of sexual dynamic these charts suggest.]',
            '',
            'MAGNETIC PULL: [0-100]',
            '[2 sentences: the initial attraction and fascination factor.]',
            '',
            'KARMIC RESONANCE: [0-100]',
            '[2 sentences: how strongly placements suggest pre-existing soul familiarity.]',
            '',
            'SHADOW RISK: [0-100]',
            '[2 sentences: the destructive potential if both collapse into worst patterns.]',
            '',
            'HEALING POTENTIAL: [0-100]',
            '[2 sentences: what these two could heal in each other if they stay conscious.]',
            '',
            'WORLD-CHANGING POTENTIAL: [0-100]',
            '[2 sentences: what these two could build or ignite externally if they combined forces.]',
        ].join('\n');
    }
    if (readingKind === 'verdict') {
        return [
...
function preferenceBand(level) {
    if (level <= 2)
        return 'stability-first (safe, grounded, low-chaos)';
    if (level <= 4)
        return 'gentle depth (emotionally warm, not extreme)';
    if (level <= 6)
        return 'balanced intensity (depth plus friction tolerance)';
    if (level <= 8)
        return 'high intensity (transformative, shadow-facing dynamics)';
    return 'extreme intensity (edge, obsession-risk, high-voltage bonds)';
}
function preferenceLens(readingKind, level) {
    const clamped = Number.isFinite(level) ? Math.min(10, Math.max(1, Math.round(level))) : 5;
...
function outputLengthContract(readingKind, systemCount, override) {
    const boundedCount = Math.max(1, Math.min(5, Math.round(systemCount || 1)));
    const contract = override || (0, outputLengthProfiles_1.getDefaultOutputLengthContract)(readingKind);
...
function soulMemoirOverride(styleLayerId) {
    if (!String(styleLayerId || '').includes('soul-memoir'))
        return '';
    return [
...
async function buildSystemBlock(systems, readingKind, directive, stats) {
    const systemIds = [];
    const sections = [];
    for (const system of systems) {
        const { layerId, mode } = resolveSystemLayerId(system, readingKind, directive);
        const file = layerRegistry_1.SYSTEM_LAYER_REGISTRY[system].files[layerId];
        const raw = await (0, layerLoader_1.loadLayerMarkdownAsync)(file);
...
async function buildVoiceOverlayBlock(systems, styleLayerId, stats) {
    if (styleLayerId !== 'writing-style-guide-incarnation-v1')
        return '';
    const sections = [];
    const voiceArchitecture = capLayer('voice-architecture-all-systems', dropSecondPersonLines(await (0, layerLoader_1.loadLayerMarkdownAsync)('style/voice-architecture-all-systems.md')), LAYER_BUDGET.voiceArchitecture, stats);
    ...
    for (const { system, file, label } of voiceInserts) {
        if (systems.includes(system)) {
            try {
                const voice = capLayer(`voice-${system}`, dropSecondPersonLines(await (0, layerLoader_1.loadLayerMarkdownAsync)(file)), LAYER_BUDGET.systemVoice, stats);
                if (voice) {
...
async function composePrompt(input) {
    ...
    const styleRaw = await (0, layerLoader_1.loadLayerMarkdownAsync)(layerRegistry_1.STYLE_LAYER_REGISTRY.files[styleLayerId]);
    ...
    const preferenceLayer = capLayer('preference-lens', preferenceLens(input.readingKind, input.relationshipPreferenceScale), LAYER_BUDGET.preferenceLens, stats);
    const outputLengthLayer = capLayer('output-length', outputLengthContract(input.readingKind, systems.length, input.outputLengthContract), LAYER_BUDGET.outputLength, stats);
    ...
    if (input.outputLanguage && (0, languages_1.isValidLanguage)(input.outputLanguage)) {
        outputLanguageContent = (0, languages_1.getLanguageInstruction)(input.outputLanguage);
    ...
    const outputLanguageLayer = capLayer('output-language', outputLanguageContent, LAYER_BUDGET.outputLanguage, stats);
    ...
    const verdictLayer = verdictLayerId
        ? capLayer('final-verdict-layer', await (0, layerLoader_1.loadLayerMarkdownAsync)(layerRegistry_1.VERDICT_LAYER_REGISTRY.files[verdictLayerId]), LAYER_BUDGET.systemKnowledge, stats)
        : '';
...
//# sourceMappingURL=composePrompt.js.map