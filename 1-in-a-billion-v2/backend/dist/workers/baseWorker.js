...
const workerCrashGuards_1 = require("./workerCrashGuards");
require("./workerCrashGuards"); // Register global handlers
class BaseWorker {
    constructor(options) {
        this.running = false;
        this.activeHeartbeats = new Map();
        this.workerId = options.workerId || `worker-${os_1.default.hostname()}-${process.pid}`;
    ...
    async claimTasks() {
        if (!supabaseClient_1.supabase)
            return [];
        const { data, error } = await supabaseClient_1.supabase.rpc('claim_tasks', {
     ...
     */
    async processSafeTask(task) {
        try {
            // Start heartbeat
            this.startHeartbeat(task.id);
            // Register context for crash handling
            (0, workerCrashGuards_1.registerTaskContext)(task.id, task.job_id);
            // Update status to processing
            await supabaseClient_1.supabase?.from('job_tasks').update({ status: 'processing' }).eq('id', task.id);
            ...
            // Mark complete
            const { error: completeError } = await supabaseClient_1.supabase.rpc('complete_task', {
                p_task_id: task.id,
                p_worker_id: this.workerId,
            ...
            try {
                const { error: failRpcError } = await supabaseClient_1.supabase.rpc('fail_task', {
                    p_task_id: task.id,
                    p_worker_id: this.workerId,
                ...
                const { data: currentTask, error: readErr } = await supabaseClient_1.supabase
                    .from('job_tasks')
                ...
                if (readErr || !currentTask) {
                    ...
                    const attempts = Number(currentTask.attempts || 0);
                    const maxAttempts = Number(currentTask.max_attempts || 3);
    ...
    async syncJobStatus(jobId) {
        ...
        try {
            ...
            const { data: jobRow } = await supabaseClient_1.supabase
                ...
                .eq('id', jobId)
                .single();
            const prevProgress = jobRow?.progress || {};
    ...
    startHeartbeat(taskId) {
        if (this.activeHeartbeats.has(taskId))
            return;
        const interval = setInterval(async () => {
            const { data, error } = await supabaseClient_1.supabase?.rpc('heartbeat_task', {
                p_task_id: taskId,
                p_worker_id: this.workerId,
            });
            if (error || !data) {
     ...
     */
    generateFriendlyFileName(params, task, artifactType, extension) {
        ...
        if ((!systemRaw || !docType) && title) {
            ...
            if (titleMatch) {
                ...
                if (!docType) {
                    ...
                    if (docTypePart.includes('overlay'))
                        docType = 'overlay';
                    else if (docTypePart.includes('person 2') || docTypePart.includes('person2'))
                        docType = 'person2';
                    else if (docTypePart.includes('person 1') || docTypePart.includes('person1'))
                        docType = 'person1';
                    else if (docTypePart.includes('verdict'))
                        docType = 'verdict';
                    else
                        docType = 'individual';
                }
     ...
     */
    async uploadTaskArtifact(task, artifact) {
        if (!supabaseClient_1.supabase)
            return;
        ...
        const extension = artifact.type === 'audio_mp3' ? 'mp3' :
        ...
        const storagePath = artifact.type === 'text'
        ...
        // Upload to Storage
        const uploadedPath = await (0, supabaseClient_1.uploadArtifact)(storagePath, artifact.buffer, artifact.contentType);
        if (!uploadedPath) {
            throw new Error('Failed to upload artifact to Storage');
        }
        // Create artifact record
        const { error: artifactError } = await supabaseClient_1.supabase.from('job_artifacts').upsert({
            job_id: task.job_id,
            task_id: task.id,
