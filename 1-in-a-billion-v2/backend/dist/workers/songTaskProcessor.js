...
exports.processSongTask = processSongTask;
const supabaseClient_1 = require("../services/supabaseClient");
const lyricsGeneration_1 = require("../services/lyricsGeneration");
const songGeneration_1 = require("../services/songGeneration");
const costTracking_1 = require("../services/costTracking");
/**
 * Process a song generation task - ONE song per document
 */
async function processSongTask(task) {
    ...
    try {
        ...
        if (!artifacts || artifacts.length === 0) {
            ...
            const { data: altArtifacts, error: altError } = await supabaseClient_1.supabase
                .from('job_artifacts')
                .select('*')
                .eq('job_id', job_id)
                .eq('artifact_type', 'text');
            if (altError)
                throw new Error(`Failed to fetch reading documents: ${altError.message}`);
            // Find the artifact matching our docNum
            const matchingArtifact = altArtifacts?.find((a) => a.metadata?.docNum === docNum ||
        ...
        if (artifact.storage_path) {
            const { data: downloadData, error: downloadError } = await supabaseClient_1.supabase
                .storage
                .from('job-artifacts')
        ...
        const lyricsResult = await (0, lyricsGeneration_1.generateLyrics)({
            personName,
            readingText,
            relationshipContext: input.relationshipContext,
            // Pass the stable system key so per-system music prompts load correctly.
            systemKey: (system || 'verdict'),
        ...
        try {
            ...
            if (Array.isArray(parsedPersonas) && parsedPersonas.length > 0) {
                ...
                console.log(`🎛️  Using dynamic persona mix: ${personas.map((p) => `${p.name}:${p.weight}%`).join(', ')}`);
            }
        }
        catch (e) {
            console.warn("Could not fetch assistant_config for personas, using default.");
        }
        const songResult = await (0, songGeneration_1.generateSong)({
            lyrics: safeLyrics,
            personName,
        ...
        if (songResult.audioBase64) {
            ...
            audioBase64 = await (0, songGeneration_1.downloadSongAudio)(songResult.audioUrl);
        }
        else {
        ...
        await (0, costTracking_1.logCost)({
            ...
            provider: 'minimax',
            model: 'music-01',
            costUsd: costTracking_1.MINIMAX_PRICING.perSong,
        ...
        const { error: artifactError } = await supabaseClient_1.supabase
            .from('job_artifacts')
            .insert({
            job_id: job_id,
            task_id: task.id,
...
//# sourceMappingURL=songTaskProcessor.js.map