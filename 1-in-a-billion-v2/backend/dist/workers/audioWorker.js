...
const minimaxTts_1 = require("../services/minimaxTts");
...
const voiceRegistry_1 = require("../i18n/voiceRegistry");
const chunkRules_1 = require("../i18n/chunkRules");
...
async function convertWavToMp3(wav) {
    ...
    const outPath = path_1.default.join(dir, 'out.mp3');
    try {
        ...
        const mp3 = await promises_1.default.readFile(outPath);
...
// AudioWorker Class
// ─────────────────────────────────────────────────────────────────────────────
class AudioWorker extends baseWorker_1.BaseWorker {
    constructor() {
        super({
    ...
    async processTask(task) {
        try {
            ...
            let text = task.input.text || task.input.content;
            ...
            if (task.input?.textArtifactPath) {
                ...
                const { data, error } = await supabaseClient_1.supabase.storage.from('job-artifacts').download(storagePath);
                if (error || !data) {
            ...
            if (supabaseClient_1.supabase) {
                try {
                    ...
                    this._jobOutputLanguage = (0, languages_1.parseLanguage)(params.outputLanguage);
                }
                catch (metaErr) {
            ...
            const jobLangStr = this._jobOutputLanguage || 'en';
            let cleanedText = (0, textCleanup_1.cleanupTextForTTS)(originalText, jobLangStr);
            cleanedText = await (0, phoneticizer_1.phoneticizeTextForTTS)(cleanedText, jobLangStr);
            const dedupResult = (0, audioProcessing_1.dedupeAdjacentSentences)(cleanedText);
            ...
            const jobLang = this._jobOutputLanguage || 'en';
            const intro = (0, spokenIntro_1.buildLocalizedSpokenIntro)({
            ...
            const langChunkConfig = (0, chunkRules_1.getChunkConfig)(jobLang);
            ...
            // ─────────────────────────────────────────────────────────────────────
            let chunks = (0, audioProcessing_1.splitIntoChunks)(text, chunkSize);
            ...
            // ─────────────────────────────────────────────────────────────────────
            if (!(0, languages_1.isValidLanguage)(jobLang)) {
            ...
            // ─────────────────────────────────────────────────────────────────────
            const activeProvider = await apiKeysHelper_1.apiKeys.activeTtsProvider() || 'replicate';
            let mp3;
            let duration;
            ...
            if (activeProvider === 'minimax') {
                ...
                let minimaxVoiceId = voice?.minimaxVoiceId;
                if (!minimaxVoiceId) {
                    const category = voice?.category || 'neutral';
                ...
                let hasRegisteredClone = !!voice?.minimaxClonedVoiceId;
                if (hasRegisteredClone) {
                    minimaxVoiceId = voice.minimaxClonedVoiceId;
                }
                else if (supabaseClient_1.supabase && !isTurboPreset) {
                    // Look up registered clone from api_keys table (set via admin endpoint)
                    try {
                        const { data: cloneKey } = await supabaseClient_1.supabase
                            .from('api_keys')
                            .select('token')
                ...
                if (!isTurboPreset && !hasRegisteredClone) {
                    ...
                    const refAudioUrl = voice?.cloneAudioUrl || voice?.sampleAudioUrl || task.input.audioUrl || this.voiceSampleUrl;
                    if (refAudioUrl) {
                        console.log(`[AudioWorker] Uploading MiniMax reference audio (purpose=prompt_audio) from: ${refAudioUrl}`);
                        clonePromptFileId = await (0, minimaxTts_1.getMinimaxSequenceForUrl)(refAudioUrl, `${voiceId}_reference.wav`).catch(e => {
                ...
                try {
                    ...
                    mp3 = await withTimeout((0, minimaxTts_1.generateMinimaxAsync)(text, minimaxVoiceId, clonePromptFileId, MINIMAX_DEFAULT_SPEED, MINIMAX_DEFAULT_VOLUME, minimaxLang), MINIMAX_HARD_TIMEOUT_MS, `MiniMax TTS (${text.length} chars, ${jobLang})`);
                }
                catch (error) {
                ...
                // ─────────────────────────────────────────────────────────────────────
                const voiceConfig = (0, voiceRegistry_1.getVoiceConfig)(jobLang);
                ...
                let replicateToken = await apiKeysHelper_1.apiKeys.replicate().catch(() => null) || env_1.env.REPLICATE_API_TOKEN;
                if (!replicateToken) {
                    // Forced refresh: avoid stale negative cache after transient Supabase/API hiccups.
                    (0, apiKeys_1.clearApiKeyCache)('replicate');
                    replicateToken = await apiKeysHelper_1.apiKeys.replicate().catch(() => null) || env_1.env.REPLICATE_API_TOKEN;
                }
                if (!replicateToken) {
                ...
                const repetitionPenaltyRaw = Number(process.env.CHATTERBOX_REPETITION_PENALTY ?? (voiceSettings.repetition_penalty ?? 1.7));
                ...
                if (useGlobalQueue) {
                    ...
                    const jobIds = await (0, replicateQueue_1.enqueueAllChunks)(taskId, chunks, replicateModel, baseInput, textField, chunkTimeoutMs);
                    ...
                    audioBuffers = await (0, replicateQueue_1.waitForAllChunks)(jobIds, replicateModel, queueTimeoutMs);
                }
                else {
                    // ═══════════════════════════════════════════════════════════════════
                    ...
                    for (let i = 0; i < chunks.length; i++) {
                        ...
                        for (let attempt = 1; attempt <= maxRetries; attempt++) {
                            try {
                                ...
                                const output = await (0, replicateRateLimiter_1.runReplicateWithRateLimit)(`audioWorker:chunk:${i + 1}`, () => withTimeout(replicate.run(replicateModel, { input }), chunkTimeoutMs, `Replicate chunk ${i + 1}`));
                ...
                // ─────────────────────────────────────────────────────────────────────
                if (audioProcessing_1.AUDIO_CONFIG.SILENCE_TRIM_ENABLED) {
                    ...
                    const trimLimit = (0, p_limit_1.default)(3);
                    const trimTasks = audioBuffers.map((buffer, i) => {
                        return trimLimit(async () => {
                            const before = buffer.length;
                            audioBuffers[i] = await (0, audioProcessing_1.trimSilenceFromWav)(buffer);
                ...
                const gapMs = audioProcessing_1.AUDIO_CONFIG.INTER_CHUNK_GAP_MS;
                if (gapMs > 0 && audioBuffers.length > 1) {
                    ...
                    const gapWav = (0, audioProcessing_1.buildSilenceWav)(gapMs);
                    const withGaps = [];
                ...
                console.log(`🎵 [AudioWorker] Final: ${Math.round(mp3.length / 1024)}KB MP3, ~${duration}s from ${chunks.length} chunks (Replicate)`);
            } // End Replicate block
            if (activeProvider === 'minimax') {
                // Log Minimax Cost asynchronously based on text length
                (0, costTracking_1.logMinimaxTtsCost)(task.job_id, task.id, text.length, `AudioWorker: Minimax (${jobLang})`)
                    .catch(e => console.error("⚠️ Failed to log Minimax cost", e));
            }
            else {
                // Log Replicate Cost based on elapsed total time
                (0, costTracking_1.logReplicateCost)(task.job_id, task.id, elapsedMs, `AudioWorker: Replicate (${voiceId})`)
