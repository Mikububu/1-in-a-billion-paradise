...
async function processNextJob() {
    ...
    try {
        ...
        const vectors = peopleRaw.map(row => ({
        ...
        if (job.job_type === 'many_to_many') {
            for (let i = 0; i < vectors.length; i++) {
                for (let j = i + 1; j < vectors.length; j++) {
                    const result = (0, vedic_ashtakoota_vectorized_engine_1.matchVedicPair)(vectors[i], vectors[j]);
                    matches.push({
            ...
            if (!source)
                throw new Error('Source profile (is_user=true) not found');
            ...
            if (candidates.length > 1000) {
                ...
                for (let i = 0; i < candidates.length; i += CHUNK_SIZE) {
                    ...
                    const chunkResults = (0, vedic_ashtakoota_vectorized_engine_1.matchBatch)(source, chunk);
                ...
                const results = (0, vedic_ashtakoota_vectorized_engine_1.matchBatch)(source, candidates);
...
//# sourceMappingURL=vedicMatchWorker.js.map