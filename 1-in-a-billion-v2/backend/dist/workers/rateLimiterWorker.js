...
exports.startRateLimiterWorker = startRateLimiterWorker;
...
function createReplicateClient() {
    ...
    return new replicate_1.default({ auth: token });
}
/* ── Timeout helper ─────────────────────────────────────────────────── */
...
async function processChunk(replicate, job) {
    const { chunkIndex, totalChunks, replicateModel, replicateInput, chunkTimeoutMs, taskId } = job.data;
    const textPreview = (replicateInput.text || replicateInput.text_to_synthesize || '').substring(0, 50);
    console.log(`[RateLimiterWorker] Processing chunk ${chunkIndex + 1}/${totalChunks} ` +
        `(task ${taskId}) - "${textPreview}..."`);
    const startTime = Date.now();
    // Call Replicate through the rate limiter
    const output = await (0, replicateRateLimiter_1.runReplicateWithRateLimit)(`rateLimiter:chunk:${chunkIndex + 1}`, () => withTimeout(replicate.run(replicateModel, { input: replicateInput }), chunkTimeoutMs, `Replicate chunk ${chunkIndex + 1}`));
    ...
    if (!supabaseClient_1.supabase)
        throw new Error('Supabase not configured - cannot upload audio chunks');
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        uploadErr = error;
        console.warn(`[RateLimiterWorker] Supabase upload attempt ${attempt}/3 failed for ${storagePath}: ${error.message}`);
        if (attempt < 3) {
    ...
    console.log(`[RateLimiterWorker] ✅ Chunk ${chunkIndex + 1}/${totalChunks} done - ` +
...
/* ── Worker setup ──────────────────────────────────────────────────── */
function startRateLimiterWorker() {
    const replicate = createReplicateClient();
    // 1. FAST WORKER (English / chatterbox-turbo)
    const fastWorker = new bullmq_1.Worker(replicateQueue_1.QUEUE_NAME_FAST, async (job) => {
    ...
    const slowWorker = new bullmq_1.Worker(replicateQueue_1.QUEUE_NAME_SLOW, async (job) => {
        return processChunk(replicate, job);
    }, {
        connection: (0, redisClient_1.createRedisConnection)(),
    ...
    const attachHandlers = (worker, prefix) => {
        worker.on('completed', (job) => {
            if (job) {
                console.log(`[${prefix}] Job ${job.id} completed (chunk ${job.data.chunkIndex + 1})`);
            }
        });
        worker.on('failed', (job, error) => {
            if (job) {
                const is429 = (0, replicateRateLimiter_1.isReplicateRateLimitError)(error);
                console.error(`[${prefix}] Job ${job.id} failed (chunk ${job.data.chunkIndex + 1}, ` +
...
//# sourceMappingURL=rateLimiterWorker.js.map