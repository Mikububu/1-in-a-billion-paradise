 ...
 */
function estimateTimezone(longitude, country) {
 ...
 */
function isValidCityType(types) {
...
router.get('/search', async (c) => {
    ...
    try {
        const googlePlacesKey = await (0, apiKeys_1.getApiKey)('google_places');
        if (!googlePlacesKey) {
        ...
        for (const strategy of searchStrategies) {
            try {
                ...
                const autocompleteResponse = await axios_1.default.get(autocompleteUrl);
                ...
                if (autocompleteData.status === 'OK' && autocompleteData.predictions) {
                    ...
                    for (const pred of autocompleteData.predictions) {
                        if (seenPlaceIds.has(pred.place_id))
                            continue;
        ...
        for (const prediction of allPredictions.slice(0, 8)) {
            try {
                ...
                try {
                    const timezoneResponse = await axios_1.default.get(timezoneUrl);
