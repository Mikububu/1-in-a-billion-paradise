...
const normalizeStoragePath = (p) => {
    ...
    if (s.startsWith('readings/'))
        return { bucketHint: 'readings', path: s.slice('readings/'.length) };
    if (s.startsWith('job-artifacts/'))
        return { bucketHint: 'job-artifacts', path: s.slice('job-artifacts/'.length) };
    return { path: s };
};
...
router.delete('/:clientPersonId', async (c) => {
    try {
        ...
        if (!clientPersonId)
            return c.json({ success: false, error: 'Missing clientPersonId' }, 400);
        ...
        const jobsToDelete = (jobs || []).filter((job) => {
            ...
            if (p1Id === clientPersonId || p2Id === clientPersonId)
                return true;
            if (personName && (p1Name === personName || p2Name === personName))
                return true;
            return false;
        }) || [];
        ...
        if (jobIds.length > 0) {
            ...
            const normalized = (artifacts || [])
                .map((a) => normalizeStoragePath(a?.storage_path))
                .filter(Boolean);
            const readingsPaths = [];
            const jobArtifactsPaths = [];
            const unknownPaths = [];
            for (const n of normalized) {
                if (n.bucketHint === 'readings')
                    readingsPaths.push(n.path);
                else if (n.bucketHint === 'job-artifacts')
                    jobArtifactsPaths.push(n.path);
                else
                    unknownPaths.push(n.path);
            }
