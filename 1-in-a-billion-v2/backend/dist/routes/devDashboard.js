...
function getBearerTokenFromReq(c) {
...
router.get('/dashboard', async (c) => {
    try {
        ...
        const legacyJobs = jobQueue_1.jobQueue.getAllJobs().map(job => ({
            id: job.id,
            type: job.type,
            status: job.status,
...
router.get('/jobs/:jobId', async (c) => {
    ...
    try {
        ...
        const legacyJob = jobQueue_1.jobQueue.getJob(jobId);
        if (legacyJob) {
            return c.json({
                success: true,
                job: {
            ...
            if (job.artifacts && Array.isArray(job.artifacts)) {
                await Promise.all(job.artifacts.map(async (artifact) => {
                    if (!artifact.public_url && artifact.storage_path) {
                        artifact.public_url = (await (0, supabaseClient_2.getSignedArtifactUrl)(artifact.storage_path)) || undefined;
                    }
                }));
            }
            return c.json({
...
router.get('/stats', async (c) => {
    try {
        const legacyJobs = jobQueue_1.jobQueue.getAllJobs();
        const legacyStats = {
            total: legacyJobs.length,
            pending: legacyJobs.filter((j) => j.status === 'pending' || j.status === 'queued').length,
            processing: legacyJobs.filter((j) => j.status === 'processing').length,
            complete: legacyJobs.filter((j) => j.status === 'complete').length,
            error: legacyJobs.filter((j) => j.status === 'error').length,
        };
        let supabaseStats = {
            total: 0,
            pending: 0,
        ...
        try {
            ...
            if (supabase) {
                ...
                if (!error && data) {
                    supabaseStats = {
                        total: data.length,
                        pending: data.filter((j) => j.status === 'pending' || j.status === 'queued').length,
                        processing: data.filter((j) => j.status === 'processing').length,
                        complete: data.filter((j) => j.status === 'complete').length,
...
router.get('/runpod/endpoints', async (c) => {
    try {
        ...
        for (const a of attempts) {
            try {
                const res = await axios_1.default.get(a.url, { headers });
                data = res.data;
                lastErr = null;
                break;
            }
        ...
        const raw = (Array.isArray(data) ? data : null) ||
            data?.serverless ||
            data?.endpoints ||
            data?.data ||
            [];
        const endpoints = (Array.isArray(raw) ? raw : []).map((e) => ({
...
//# sourceMappingURL=devDashboard.js.map