...
function extractCoverQuote(reading: string): string {
  ...
  const cleaned = withoutHeadlines || raw.replace(/\s+/g, ' ').trim();
...
async function main() {
  ...
  logRunConfig({ outDir, portraitsDir, styleLayerId, requestedSystem, requestedJobs });

  const supportedSystems: SystemId[] = ['western', 'vedic'];
