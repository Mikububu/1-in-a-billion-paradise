...
import { SYSTEMS as NUCLEAR_V2_SYSTEMS, SYSTEM_DISPLAY_NAMES as NUCLEAR_V2_SYSTEM_NAMES, type SystemName as NuclearV2SystemName, NUCLEAR_DOCS, VERDICT_DOC, buildOverlayPrompt as buildNuclearV2OverlayPrompt, buildVerdictPrompt, buildPersonPrompt } from '../prompts/structures/paidReadingPrompts';
...
const startJobSchema = z.object({
  type: z.enum(['extended', 'synastry', 'bundle_verdict']),
  ...
  relationshipPreferenceScale: z.number().min(1).max(10).default(5),
  // Must stay in sync with the mobile UI circle input max length.
  relationshipContext: z.string().max(600).optional(), // Contextual infusion for overlay interpretation
  personalContext: z.string().max(600).optional(), // Contextual infusion for individual readings
  ...
  outputLengthContract: z.any().optional(),
  bundleComposition: z.any().optional(),
  promptLayerDirective: z.any().optional(),
});
...
jobQueue.registerProcessor('extended', async (job, updateProgress) => {
  ...
  try {
    ...
    for (let i = 0; i < totalSystems; i++) {
      ...
      const reading = await llmPaid.generate(prompt, `extended-${system}`, { maxTokens: 16384 });
...
jobQueue.registerProcessor('synastry', async (job, updateProgress) => {
  ...
  try {
    ...
    const reading = await llmPaid.generate(prompt, `synastry-${system}`, { maxTokens: 16384 });
...
jobQueue.registerProcessor('nuclear_v2', async (job, updateProgress) => {
  ...
  try {
    ...
    for (const system of NUCLEAR_V2_SYSTEMS) {
      ...
      const p1Text = await llmPaid.generate(p1Prompt, `nuclear_v2-${system}-p1`, { maxTokens: 16384 });
      ...
      const p2Text = await llmPaid.generate(p2Prompt, `nuclear_v2-${system}-p2`, { maxTokens: 16384 });
    ...
    for (const system of NUCLEAR_V2_SYSTEMS) {
      ...
      const overlayText = await llmPaid.generate(overlayPrompt, `nuclear_v2-${system}-overlay`, { maxTokens: 16384 });
    ...
    const verdictText = await llmPaid.generate(verdictPrompt, 'nuclear_v2-verdict', { maxTokens: 16384 });
