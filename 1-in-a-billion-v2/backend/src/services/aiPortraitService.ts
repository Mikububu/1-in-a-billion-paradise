...
function isRetryableError(error: any): boolean {
  ...
  return (
    /fetch failed|terminated|timeout|ETIMEDOUT|ECONNRESET|socket|Bad Gateway|gateway/i.test(message) ||
    status >= 500
  );
}

async function generateGoogleImageWithRetry(
  ai: GoogleGenAI,
  parts: any[],
  maxAttempts = 4
): Promise<any> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      console.warn(
        `⚠️ [AI Portrait] Google image call failed (attempt ${attempt}/${maxAttempts}): ${error?.message || error}. Retrying in ${delayMs}ms...`
      );
      await sleep(delayMs);
    }
  }
...
async function uploadPortraitWithRetry(
  supabase: any,
  storagePath: string,
  imageBuffer: Buffer,
  maxAttempts = 4
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    console.warn(
      `⚠️ [AI Portrait] Upload failed (attempt ${attempt}/${maxAttempts}): ${error.message || error}. Retrying in ${delayMs}ms...`
    );
    await sleep(delayMs);
  }
