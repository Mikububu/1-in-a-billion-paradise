...
1. Registration moved to post-Languages (pre-payment capture).
...
1. "How matching works" should be an overlay, not a standalone screen route.
...
1. Do not migrate orphan route-only legacy screens.
...
1. Aggressive simplification rule for migration.
...
1. Remove unused `PeopleList` selection mode in V2.
...
1. Keep `ChatList` as a dedicated screen.
...
1. Vedic matchmaking spec is a hard migration authority.
...
1. Spice preference is a soft Vedic ranking lens (not a hard gate).
...
1. Leather texture is global (single source), not per-screen duplicated.
...
1. Reading-job tap flow always enters JobDetail status screen first.
...
1. Western prompt layer naming migrated to Hellenistic.
...
1. Partner reading route is explicit from SynastryOptions.
...
1. Remove legacy `style: production` from job payloads.
...
1. Remove orphan `relationshipMode` from partner hook payloads.
...
1. Entitlement must refresh on app foreground.
...
1. Remove startup job-buffer cap enforcement from app shell.
...
1. Prompt markdown source-of-truth is author-provided files synced into V2.
...
1. Birth-data continue tap hardened against keyboard double-tap loss.
...
1. Post-Languages back-navigation lock in account capture flow.
...
1. Normalize asset requires to deterministic relative paths.
...
1. Remove legacy "Nuclear" wording from prompt docs.
...
1. Vedic prompt merge policy: keep Jyotish depth, remove global style overlap.
...
1. Human Design + Gene Keys prompt layers aligned to analysis-only contract.
...
1. Hellenistic + Kabbalah prompts re-curated to remove embedded style instructions.
...
1. Image-generation prompts moved to editable markdown blocks in V2.
...
1. Restore synastry couple-image generation trigger in V2 overlay start flow.
...
1. Synastry preview now shows tappable couple image (when available).
...
1. V2 backend image module scaffold added (prompt-aware, validator-first).
...
1. V2 backend PDF runtime migrated as config-driven, in-memory generator.
...
1. Remove orphan subscription/upsell scaffolding from V2.
...
1. Remove unused frontend Vedic matchmaking client shim.
...
1. Job receipt buffer policy simplified to retention-only.
...
1. Prompt layer registry now uses Hellenistic IDs only.
...
1. Dynamic LLM Phoneticizer before TTS audio generation.

- Decision: added `backend/src/services/text/phoneticizer.ts` and integrated it into `backend/src/workers/audioWorker.ts` and `backend/src/routes/audio.ts`.
- Rationale: ensure accurate pronunciation of esoteric English terms (Human Design, Gene Keys) by foreign TTS voices without relying on hardcoded dictionaries.
