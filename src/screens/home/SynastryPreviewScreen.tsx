...
export const SynastryPreviewScreen = ({ navigation, route }: Props) => {
  ...
  const [loadingCoupleImage, setLoadingCoupleImage] = useState(false);
  const [coupleImagePreviewVisible, setCoupleImagePreviewVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    const person1Id = profileUser?.id;
    const person2Id = partnerPerson?.id;
    const portrait1 = profileUser?.portraitUrl;
    const portrait2 = partnerPerson?.portraitUrl;

    if (!person1Id || !person2Id || !portrait1 || !portrait2) {
      setCoupleImageUrl(null);
      setLoadingCoupleImage(false);
      return;
    }

    const loadCoupleImage = async () => {
      setLoadingCoupleImage(true);
      const result = await getCoupleImage(person1Id, person2Id, portrait1, portrait2, false);
      if (cancelled) return;
      if (result.success && result.coupleImageUrl) {
        setCoupleImageUrl(result.coupleImageUrl);
      } else {
        setCoupleImageUrl(null);
      }
      setLoadingCoupleImage(false);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Modal
        visible={coupleImagePreviewVisible && Boolean(coupleImageUrl)}
        transparent
        animationType="fade"
        onRequestClose={() => setCoupleImagePreviewVisible(false)}
      >
        <Pressable style={styles.couplePreviewBackdrop} onPress={() => setCoupleImagePreviewVisible(false)}>
          <View style={styles.couplePreviewCard}>
            <Text style={styles.couplePreviewTitle}>{userName} & {partner}</Text>
            {coupleImageUrl ? (
