...
export const PostHookOfferScreen = ({ navigation }: Props) => {
  ...
  const pages = useMemo<OfferPage[]>(
    () => [
      ...
      {
        title: 'One complete\npersonal reading.\n(15-30 min audio & 5-8 pages approx)',
      },
      {
  ...
  useEffect(() => {
    ...
    const loadPrice = async () => {
      ...
      const price = yearly?.product?.priceString || yearly?.storeProduct?.priceString || null;
  ...
  const buttonLabel = useMemo(() => {
    ...
    if (yearlyPrice) return `Yes let me in (${yearlyPrice}/yr)`;
