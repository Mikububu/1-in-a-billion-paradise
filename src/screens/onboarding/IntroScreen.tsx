...
export const IntroScreen = ({ navigation }: Props) => {
  ...
  const canEnterDashboard = useCallback(async () => {
    ...
    // Product rule: keep app access even if subscription is inactive.
    // Home/Gallery show a short renewal warning; chat access is gated until renewal.
    if (verification.success && !verification.active) {
