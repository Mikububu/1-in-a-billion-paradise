...
export const CoreIdentitiesScreen = ({ navigation }: Props) => {
  ...
  const runSequence = async () => {
    ...
    // ============================================================================
    // AUDIO GENERATION PIPELINE (REQUIREMENT #3, #4)
    // ============================================================================
    // DESIGN: Generate ALL audio here, wait for ALL to complete before navigating
    //
    // Timeline:
    //   INTRO (10s)   → Fetch SUN reading → Start SUN audio generation
    //   SUN (4s)      → Fetch MOON reading → Start MOON audio generation
    //   MOON (4s)     → Fetch RISING reading → Start RISING audio generation
    //   RISING (3s)   → Wait for ALL audio → Navigate to HookSequence
    //
    // Audio Generation Strategy:
    //   - ALL audio: Generated here, MUST complete before navigation
    ...
    try {
      ...
      const sunFetchPromise = fetch(`${env.CORE_API_URL}/api/reading/sun?provider=deepseek${nocacheParam}`, {
        method: 'POST',
      ...
      const moonFetchPromise = fetch(`${env.CORE_API_URL}/api/reading/moon?provider=deepseek${nocacheParam}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      }).then(async (res) => {
      ...
      const risingFetchPromise = fetch(`${env.CORE_API_URL}/api/reading/rising?provider=deepseek${nocacheParam}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      }).then(async (res) => {
      ...
      // ========== NAVIGATION WITH STACK RESET (REQUIREMENT #5, #7) ==========
