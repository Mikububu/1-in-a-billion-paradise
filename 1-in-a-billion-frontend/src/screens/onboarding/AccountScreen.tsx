...
export const AccountScreen = ({ navigation, route }: Props) => {
  ...
  // Name input state (replaces NameInputScreen)
  ...
  const handleSupabaseExchange = async (idToken: string, provider: 'apple' | 'google') => {
    try {
      ...
      // Use name from local input field (name field is now on this screen)
      const trimmedName = name.trim();
      const firstName = trimmedName.split(' ')[0];
      if (firstName) {
        setDisplayName(firstName);
        useOnboardingStore.getState().setName(firstName);
        console.log(`✅ Using name from AccountScreen: ${firstName}`);
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Save name before OAuth flow (name field is now on this screen)
      const trimmedName = name.trim();
      const firstName = trimmedName.split(' ')[0];
      if (firstName) {
        setDisplayName(firstName);
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}${endpoint}`, {
        ...
        body: JSON.stringify({
          ...
          name: firstName, // Use name from local input field
        }),
      });
      ...
      if (data.session) {
        ...
        // Name already saved to stores at the start of handleEmailAuth
      ...
      >
        ...
        >
          ...
          <TextInput
            ...
            autoCorrect={false}
            returnKeyType="done"
            editable={!isLoading && emailAuthState !== 'loading'}
          />

          {/* Sign-up buttons - only show when name is entered */}
          {hasValidName && !showEmailInput ? (
            <>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  authSection: {
    gap: spacing.sm,
    paddingTop: Platform.OS === 'ios' ? 160 : 140, // More padding to move content down
