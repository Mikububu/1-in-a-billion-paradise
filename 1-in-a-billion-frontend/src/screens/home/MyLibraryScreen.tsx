...
export const MyLibraryScreen = ({ navigation }: Props) => {
  ...
  const allPeopleWithReadings = useMemo<LibraryPerson[]>(() => {
    ...
    // Add people from job receipts (person1/person2) - include ALL job statuses
    queueJobsNewestFirst
      ...
      .forEach((job: any) => {
        ...
        // For any job without person name, use a fallback
        const p1Name = params.person1?.name || `Reading ${job.id.slice(0, 8)}`;
        const p2Name = params.person2?.name || (params.person2 ? 'Partner' : undefined);
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            // OPTIMISTIC DELETION: Remove from UI immediately for instant feedback
            setQueueJobs(prev => prev.filter((j: any) => j.id !== jobId));
            ...
            try {
              ...
              if (sessionError || !accessToken) {
                ...
                // Trigger refresh to restore the job if deletion failed
                setQueueRefreshKey(prev => prev + 1);
                return;
              }
              ...
              // Trigger full refresh to sync with backend
              setQueueRefreshKey(prev => prev + 1);
