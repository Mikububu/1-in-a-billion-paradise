...
export const DeepReadingReaderScreen = ({ navigation, route }: Props) => {
  ...
  const allMediaReady = useMemo(() => {
    ...
    // Song is optional - don't require it for the reading to be considered "ready"
  ...
  const playAudio = useCallback(async () => {
    const audio = getFirst('audio_m4a') || getFirst('audio_mp3') || getFirst('audio');
    if (!audio) return Alert.alert('Audio not ready', 'No audio artifact is available yet.');
    const docNum = audio.metadata?.docNum || 1;
  ...
  const playSong = useCallback(async () => {
    ...
    const docNum = song.metadata?.docNum || 1;
    const url = await getPlayableArtifactUrl(jobId, docNum, 'song', song.storage_path);
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.textCard, { position: 'relative' }]}>
          {blocks.length === 0 ? (
            ...
            blocks.map((b, idx) =>
              b.kind === 'heading' ? (
                <Text key={idx} style={[styles.heading, !allMediaReady && { opacity: 0.5 }]} selectable>{b.text}</Text>
              ) : (
                <Text key={idx} style={[styles.textBody, !allMediaReady && { opacity: 0.5 }]} selectable>{b.text}</Text>
              )
            )
          )}
        </View>
