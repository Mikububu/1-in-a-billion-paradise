...
export const SynastryPreviewScreen = ({ navigation, route }: Props) => {
  ...
  // Partner signs (would come from API - using demo data for now)
  const partnerSigns = {
    sun: 'Pisces',
    moon: 'Cancer',
    rising: 'Scorpio',
  ...
  useEffect(() => {
    const calculateScores = async () => {
      ...
      if (!userBirthDate || !userBirthCity || !partnerBirthDate || !partnerBirthCity) {
        ...
        setScoreError('Add birth date + city for both people to calculate compatibility.');
      ...
      if (!userBirthTime || !partnerBirthTime) {
        ...
        setScoreError('Birth time is required to calculate compatibility (Rising sign).');
        setLoadingScores(false);
        setSpicyScore(null);
        setSafeStableScore(null);
        return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (typeof data.spicyScore === 'number' && typeof data.safeStableScore === 'number') {
            setSpicyScore(data.spicyScore);
            setSafeStableScore(data.safeStableScore);
            ...
            setSpicyScore(null);
            setSafeStableScore(null);
            setScoreError('Could not calculate compatibility scores (invalid response).');
          }
        } else {
          ...
          setSpicyScore(null);
          setSafeStableScore(null);
          setScoreError('Could not calculate compatibility scores (API error).');
        }
      } catch (error) {
        console.error('❌ Error calculating compatibility:', error);
        setSpicyScore(null);
        setSafeStableScore(null);
        setScoreError('Could not calculate compatibility scores (network error).');
      } finally {
        setLoadingScores(false);
      }
    };
    ...
    calculateScores();
  }, [userBirthDate, userBirthTime, userBirthCity, partnerBirthDate, partnerBirthTime, partnerBirthCity, userName, partner, authUser?.id, markFreeOverlayUsed]);
  ...
  const insights = [
    {
      ...
      content: `Your ${userSigns.sun} Sun meets ${partner}'s ${partnerSigns.moon} Moon. This creates a dynamic where your core identity resonates with ${partner}'s emotional nature. You see the world through logic and service, while ${partner} feels it through nurturing intuition.`,
    },
    {
      title: 'Rising Sign Chemistry',
      content: `Your ${userSigns.rising} Rising meets ${partner}'s ${partnerSigns.rising} Rising. First impressions matter - you appear adventurous and philosophical, while ${partner} projects mystery and intensity. This creates magnetic attraction or curious tension.`,
    },
    {
              ...
              <View style={styles.connectionViz}>
                ...
                <View style={styles.personBubble}>
                  ...
                  <View style={styles.signsRow}>
                    <Text style={styles.personSigns}>☉ {userSigns.sun}</Text>
                    <Text style={styles.signDot}>·</Text>
                    <Text style={styles.personSigns}>☽ {userSigns.moon}</Text>
                    <Text style={styles.signDot}>·</Text>
                    <Text style={styles.personSigns}>↑ {userSigns.rising}</Text>
                  </View>
                </View>
                ...
                <View style={styles.personBubble}>
                  ...
                  <View style={styles.signsRow}>
                    <Text style={styles.personSigns}>☉ {partnerSigns.sun}</Text>
                    <Text style={styles.signDot}>·</Text>
                    <Text style={styles.personSigns}>☽ {partnerSigns.moon}</Text>
                    <Text style={styles.signDot}>·</Text>
                    <Text style={styles.personSigns}>↑ {partnerSigns.rising}</Text>
                  </View>
                </View>
              </View>
            </Animated.View>
