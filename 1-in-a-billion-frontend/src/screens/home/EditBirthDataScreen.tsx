...
export const EditBirthDataScreen = ({ navigation, route }: Props) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ Updating existing self profile with edited birth data');
      updatePerson(target.id, {
        birthData: nextBirthData,
        // If birth time changed, any cached placements are now unreliable.
        placements: birthTimeChanged ? undefined : target.placements,
      });

      // Keep onboarding store in sync for main user (so other screens don’t drift)
      if (target.isUser) {
  ...
  }, [addPerson, canSave, dateValue, getPerson, navigation, person, selectedCity, setBirthCity, setBirthDate, setBirthTime, timeValue, updatePerson]);
