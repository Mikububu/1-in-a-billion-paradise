  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // MIGRATION: Clear old legacy Supabase keys (one-time on app launch)
        // This ensures users with old EAS bundles get fresh auth state
        const migrationKey = 'supabase_key_migration_v2';
        const hasMigrated = await AsyncStorage.getItem(migrationKey);
        if (!hasMigrated) {
          console.log('🔄 Bootstrap: Running Supabase key migration...');
        ...
        if (session) {
          ...
          // CRITICAL SECURITY FIX: Check if user ID changed (different user logged in)
          // If so, clear profile store to prevent data leakage between accounts
          const currentUserId = useAuthStore.getState().user?.id;
          if (currentUserId && currentUserId !== session.user.id) {
            console.log('🚨 SECURITY: User ID changed - clearing profile store to prevent data leak');
            console.log(`   Old user: ${currentUserId.substring(0, 8)}`);
            console.log(`   New user: ${session.user.id.substring(0, 8)}`);
            
            // Import and clear profile store
            const { useProfileStore } = await import('@/store/profileStore');
            useProfileStore.getState().reset();
            
            // Also clear subscription store
            const { useSubscriptionStore } = await import('@/store/subscriptionStore');
            useSubscriptionStore.getState().reset();
            
            console.log('✅ Profile and subscription stores cleared');
          }
          ...
          try {
            ...
            if (!profileError && profiles && profiles.length > 0) {
              ...
              if (hasHookReadings) {
                ...
                // CRITICAL: Only set hasCompletedOnboarding, NOT showDashboard
                // Per ARCHITECTURE.md: App always lands on Intro. User must tap "My Secret Life" to reach Dashboard.
                // showDashboard should only be set by user action (tapping "My Secret Life" on IntroScreen)
                const store = useOnboardingStore.getState();
                if (!store.hasCompletedOnboarding) {
                  // Use direct set to avoid triggering showDashboard
                  useOnboardingStore.setState({ hasCompletedOnboarding: true });
                }
                console.log('✅ Bootstrap: hasCompletedOnboarding set to true (showDashboard unchanged)');
              } else {
