...
// Get folder path for a specific job/doc (deterministic - no timestamp)
const getArtifactFolder = (jobId: string, docNum: number, personName?: string, system?: string): string => {
  ...
  // Simple flat structure: baseDir/jobId/docNum/
  const safeJobId = sanitize(jobId);
  return `${baseDir}${safeJobId}/${docNum}/`;
};

// Ensure directory exists (with retry for race conditions)
const ensureDir = async (dirPath: string, retries = 3): Promise<boolean> => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error('[ArtifactCache] Failed to create directory after retries:', dirPath, e);
      return false;
    }
  }
  return false;
