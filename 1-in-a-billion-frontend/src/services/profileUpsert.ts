...
export async function upsertSelfProfileToSupabase(params: UpsertSelfProfileParams): Promise<{
    ...
    try {
        ...
        // Use upsert with onConflict on user_id to avoid duplicate key errors
        const { error } = await withTimeout(supabase
            .from(TABLE_PEOPLE)
            .upsert(profileRow, {
                onConflict: 'user_id', // Conflict on user_id alone for self profiles
                ignoreDuplicates: false, // Update existing rows
            }), 8000);

        if (error) {
            console.warn('⚠️ Failed to upsert self profile (non-blocking):', error.message);
            return { success: false, error: error.message };
        }

        console.log('✅ Self profile upserted to Supabase successfully');
        return { success: true };
    } catch (err) {
