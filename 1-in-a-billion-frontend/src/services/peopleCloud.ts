...
export async function syncPeopleToSupabase(userId: string, people: Person[]) {
  ...
  // Handle self profiles - use composite key (user_id, client_person_id)
  // The database only has UNIQUE(user_id, client_person_id), not UNIQUE(user_id) alone
  if (selfProfiles.length > 0) {
    const { error } = await supabase
      .from(TABLE_PEOPLE)
      .upsert(selfProfiles, {
        onConflict: 'user_id,client_person_id', // Must use composite key
    ...
    console.log(`✅ Self profile synced to Supabase (${selfProfiles.length} profile)`);
  }
  
  // Upsert partner profiles using composite key
