...
export function checkInvariant1_SessionAuthority(): {
  ...
  logDebug({
    ...
    message: 'Checking INVARIANT 1: Session Authority',
  ...
  return {
    passed: true,
    guidance: 'If hasSession = true, user is authenticated. Navigation must respect this.',
  };
}
...
export function checkInvariant2_OnboardingFlag(): {
  ...
  const hookReadings = useOnboardingStore.getState().hookReadings;
  ...
  logDebug({
    ...
    message: 'Checking INVARIANT 2: Onboarding Flag',
    data: { hasCompletedOnboarding, hasHookReadings },
  });

  // Check if routing logic might be using hasHookReadings instead of hasCompletedOnboarding
  // This is a warning, not a hard violation
  if (hasHookReadings && !hasCompletedOnboarding) {
    return {
      passed: true, // Not a violation, just a state
      guidance: 'User has hook readings but onboarding not complete. This is expected during onboarding flow.',
    };
  }

  return {
    passed: true,
    guidance: 'hasCompletedOnboarding is the ONLY flag that determines Dashboard vs Onboarding. DO NOT check hasHookReadings for routing.',
  };
}

/**
 * INVARIANT 3: Bootstrap Sets Onboarding Flag
 * Only sets hasCompletedOnboarding = true if user has hook readings in Supabase
 */
export function checkInvariant3_BootstrapFlag(): {
  ...
  logDebug({
    ...
    message: 'Checking INVARIANT 3: Bootstrap Flag Setting',
    data: { hasCompletedOnboarding, hasHookReadings },
  });
  ...
  return {
    passed: true,
    guidance: 'Bootstrap should only set hasCompletedOnboarding = true if user has hook_readings in Supabase. Check useSupabaseAuthBootstrap.ts',
  };
}
...
export function checkInvariant4_NoProfileRouting(): {
  ...
  return {
    passed: true,
    guidance: 'Profile existence in Supabase does NOT determine routing. Only hasCompletedOnboarding determines routing.',
  };
}
