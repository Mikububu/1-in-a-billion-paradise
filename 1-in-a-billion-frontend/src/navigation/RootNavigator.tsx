...
import { View, Alert, TouchableOpacity, Text, AppState } from 'react-native';
...
const MainNavigator = () => {
  ...
  // Enforce 40-job buffer: prune receipts + delete local media for oldest jobs when over cap.
  useEffect(() => {
    if (!hasHydrated) return;
    enforceJobBufferCap().catch(() => {});
  }, [hasHydrated]);
...
export const RootNavigator = () => {
  ...
  // 3. Realtime artifact sync - instantly downloads audio/PDF/song when ready
  // Uses Supabase Realtime (no polling) for immediate downloads
  useRealtimeSubscription();

  // ============================================================================
  // ROUTING INVARIANT (DO NOT MODIFY)
  // ============================================================================
  // RULE: App always opens on Screen 1 (Intro).
  //
  // - If no session → Intro (Log In / Get Started)
  // - If session exists → Intro (Log Out / My Secret Life)
  // - Only "My Secret Life" switches to MainNavigator (showDashboard = true)
  //
  // Logged-in users NEVER auto-route to Dashboard on reopen.
  // Onboarding is only accessible after log out.
  // ============================================================================

  const hasSession = !!user;
  // Onboarding state - showDashboard controls transition to MainNavigator
  // CRITICAL: Default to false until hydration completes to prevent dashboard flash
  const rawShowDashboard = useOnboardingStore((s) => s.showDashboard);
  const setShowDashboard = useOnboardingStore((s) => s.setShowDashboard);
  const isHydrated = useOnboardingStore((s) => s._hasHydrated);
  
  // Only allow showDashboard=true AFTER hydration completes
  // This prevents the flash where old persisted state shows dashboard before reset
  const showDashboard = isHydrated ? rawShowDashboard : false;
  ...
  // ROUTING LOGIC PREP: We intentionally do NOT auto-route logged-in users to dashboard.
  ...
  // ============================================================================
  // INACTIVITY-BASED ROUTING
  // ============================================================================
  // Rule: Brief absence (< 24 hours) = restore same screen state
  //       Long absence (>= 24 hours) = reset to Intro screen
  // 
  // When app goes to background: record timestamp
  // When app returns to foreground: check elapsed time
  // ============================================================================
  const INACTIVITY_THRESHOLD_MS = 24 * 60 * 60 * 1000; // 24 hours in milliseconds
  const lastActiveTimestamp = useOnboardingStore((s) => s.lastActiveTimestamp);
  const setLastActiveTimestamp = useOnboardingStore((s) => s.setLastActiveTimestamp);
  const updateLastActive = useOnboardingStore((s) => s.updateLastActive);

  useEffect(() => {
    const sub = AppState.addEventListener('change', (nextState) => {
      if (nextState === 'background') {
        // Record when user left the app
        updateLastActive();
        console.log('📱 App backgrounded - timestamp recorded');
      } else if (nextState === 'active') {
        // App returned to foreground - check how long user was away
        const now = Date.now();
        const lastActive = lastActiveTimestamp;
        
        if (lastActive) {
          const elapsed = now - lastActive;
          const elapsedHours = Math.round(elapsed / (1000 * 60 * 60) * 10) / 10;
          
          if (elapsed >= INACTIVITY_THRESHOLD_MS) {
            // Long absence (>= 24 hours) - reset to Intro
            console.log(`📱 App resumed after ${elapsedHours}h - resetting to Intro`);
            setShowDashboard(false);
          } else {
            // Brief absence (< 24 hours) - restore same screen
            console.log(`📱 App resumed after ${elapsedHours}h - restoring previous state`);
            // Don't change showDashboard - keep current state
          }
        }
        // Update timestamp for next background event
        updateLastActive();
      }
    });
    return () => {
  ...
  }, [setShowDashboard, lastActiveTimestamp, setLastActiveTimestamp, updateLastActive]);
  ...
  // ROUTING LOGIC:
  // 1. No session → Intro
  // 2. Session + showDashboard → MainNavigator
  // 3. Session + !showDashboard → Intro (My Secret Life gate)

  if (hasSession && showDashboard) {
    console.log('✅ ROUTING: Session exists + showDashboard → MainNavigator (Dashboard)');
  } else if (hasSession) {
    console.log('✅ ROUTING: Session exists → Intro (My Secret Life gate)');
  } else {
    console.log('✅ ROUTING: No session → Intro');
  }
