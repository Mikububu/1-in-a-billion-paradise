...
export interface LyricsGenerationInput {
  personName: string;
  readingText: string; // Combined text from all reading documents
...
export async function generateLyrics(input: LyricsGenerationInput): Promise<LyricsResult> {
  const { personName, readingText, relationshipContext } = input;
...
Generate a personalized song for ${personName} based on their deep soul reading. The song must:
