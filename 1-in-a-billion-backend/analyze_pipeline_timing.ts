...
async function analyzePipelineTiming() {
  ...
  for (const job of jobs) {
    ...
    if (tasks && tasks.length > 0) {
      ...
      const taskTimings: Array<{ type: string; duration: number; status: string }> = [];
      ...
      for (const task of tasks) {
        ...
        console.log(`     ${statusIcon} [${task.sequence}] ${task.task_type}: ${task.status} (${Math.floor(taskDuration / 60)}m ${taskDuration % 60}s)`);
      ...
      // Calculate pipeline phases
      console.log(`\n   Pipeline Analysis:`);
      const textTasks = taskTimings.filter(t => t.type === 'text_generation');
      const pdfTasks = taskTimings.filter(t => t.type === 'pdf_generation');
      const audioTasks = taskTimings.filter(t => t.type === 'audio_generation' || t.type === 'audio_mp3' || t.type === 'audio_m4a');
      const songTasks = taskTimings.filter(t => t.type === 'song_generation');
