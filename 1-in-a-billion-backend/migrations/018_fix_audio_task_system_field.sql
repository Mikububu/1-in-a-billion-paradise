-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION 018: Fix audio_generation task missing system/docType fields
-- ═══════════════════════════════════════════════════════════════════════════
--
-- PROBLEM: Audio tasks were missing 'system' and 'docType' fields in input,
--          causing all audio files to be named with 'Western' regardless of
--          the actual system (Vedic, Human Design, Gene Keys, Kabbalah).
--
-- FIX: Add system and docType to audio_generation task input (like PDF tasks)
--
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    -- Audio task (sequence 200-215) - NOW INCLUDES system AND docType!
    IF v_audio_already_enqueued = 0 THEN
      INSERT INTO job_tasks (
        ...
        jsonb_build_object(
          ...
          'sourceTaskId', v_text_task.id::text,
          'docNum', COALESCE((v_text_task.output->>'docNum')::int, NULL),
          'docType', COALESCE(v_text_task.output->>'docType', NULL),  -- *** ADDED ***
          'system', COALESCE(v_text_task.output->>'system', NULL),    -- *** ADDED ***
          'voiceId', v_voice_id,
